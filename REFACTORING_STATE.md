...
- Adjudication O fix list PENDING: doc amendments (§11.2, §16.1, §16.6, §20.9-1/-2, §24.2, §25.3, §25.6, §25.8, new §27) assigned to `arch-record-O`; code changes (`resolve.rs:7` + generation wrap, `glyph.rs` whole-set setters + ASCII typed sets, `builder.rs::ascii_glyphs`, `junie.rs` shadowed array slots, `tests/perf.rs` O4 corrections, `perf_baseline.txt` header) assigned after `components-finish` returns, because both would touch `tests/perf.rs`.
