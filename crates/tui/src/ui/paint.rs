...
impl Ui<'_> {
    ...
    /// Blend one edge row toward `container`: RGB foregrounds mix
    /// componentwise, other cells on the outer row dim, and cells on another
    /// plane or reversed stay whole. Glyphs never change.
...
/// Blend one foreground channel toward the background, keeping `keep` of
/// the contrast with componentwise rounding (oracle `fade.rs`).
