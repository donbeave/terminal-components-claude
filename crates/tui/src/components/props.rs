...
/// Static label / value rows painted by the same two-column primitive as
/// [`PropsList`].
///
/// ## Construction
...
///
/// ## Layout
/// One row per pair; labels use [`RowUi::label`] and values use the trailing
/// [`RowUi::meta`] column. `measure` is `(widest label + 2 + widest value,
/// rows)`; `draw` returns the rows painted, clipped to `area`.
///
/// ## Parts
/// `LABEL` (the label column), `META` (the value column).
///
/// ## Overrides
...
mod tests {
    ...
    fn secret_row(fact: &SecretFact, row: &mut RowUi<'_>) {
        row.label("Token");
        let mut value = row.part(Part::META, 12);
        fact.secret
