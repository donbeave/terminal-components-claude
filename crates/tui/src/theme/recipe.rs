...
impl Recipes {
    ...
    /// The recipe a family that `define_family` never declared resolves
    /// through (§11.2) the neutral row-like set `CONTAINER / GUTTER / MARKER /
    /// LABEL / META …`, so `Family::custom("x")` renders instead of
    /// resolving to an empty style.
