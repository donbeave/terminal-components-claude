//! External non-Display consumer of Tree's full-row renderer.
use junie_tui::{
    App, Cx, Family, Id, ItemKey, KeyCode, Part, Rect, Response, Role, StateFlags, StylePatch,
    Tree, TreeState, Ui, Variant,
};
...
const ID: Id = Id::root("custom.tree");
struct Step {
    key: u64,
    title: &'static str,
}
...
fn paint(ui: &mut Ui<'_>, row: Rect, flags: StateFlags, _: ItemKey, step: &Step) {
    let style = ui
        .style(Family::TREE, Variant::DEFAULT, Part::CONTAINER, flags)
        .style;
    ui.fill(row, style);
...
fn ancestor_clip_preserves_logical_alignment_and_skips_invisible_callbacks() {
    ...
    scene.draw(|ui, _| {
        ui.with_area(Rect::new(3, 2, 5, 1), |ui| {
            ...
            Tree::new(ID).render_row(&renderer).key(key).draw(
                ui,
                Rect::new(1, 1, 20, 4),
                &page.state,
                &page.items,
...
#[test]
fn custom_prefix_keeps_shared_disclosure_hit_and_branch_identity() {
    // The standalone Page below uses a hierarchy and exactly the same custom painter.
    struct BranchPage {
        state: TreeState,
        items: Vec<Step>,
    }
    fn hierarchy(item: &Step) -> junie_tui::TreeNode {
        if item.key == 0 {
            junie_tui::TreeNode::parent(0)
        } else {
            junie_tui::TreeNode::leaf(1)
        }
    }
    fn branch_paint(ui: &mut Ui<'_>, row: Rect, flags: StateFlags, _: ItemKey, item: &Step) {
        let style = ui
            .style(Family::TREE, Variant::DEFAULT, Part::CONTAINER, flags)
            .style;
        ui.fill(row, style);
        if item.key == 0 {
            ui.paint_str(
                ...
                if flags.contains(StateFlags::EXPANDED) {
                    "▾"
                } else {
                    "▸"
                },
    ...
    impl App for BranchPage {
        fn update(&mut self, cx: &mut Cx<'_>) -> Response<()> {
            Tree::new(ID)
                .render_row(&branch_paint)
                .node(&hierarchy)
                .key(key)
                .update(cx, &mut self.state, &self.items)
                .erase()
    ...
    assert!(!h.app().state.is_expanded(ItemKey::num(0)));
    assert_eq!(h.cell(2, 1).symbol(), "▸");
    let _ = h.click(2, 1);
    assert_eq!(h.cell(2, 1).symbol(), "▾");
    assert!(h.app().state.is_expanded(ItemKey::num(0)));
    let _ = h.key(KeyCode::Right);
    assert_eq!(h.app().state.cursor(), Some(ItemKey::num(1)));
    let _ = h.key(KeyCode::Left);
    assert_eq!(h.app().state.cursor(), Some(ItemKey::num(0)));
}
