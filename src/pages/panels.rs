...
impl Page for PanelsPage {
    ...
    fn render(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut RenderCtx) {
        ...
        for (i, line) in crate::ui::text::wrap("A card is a filled surface. Its title sits in the top-left and metadata on the right. It never has a border.", inner.width as usize).iter().enumerate() {
        ...
        for (i, line) in crate::ui::text::wrap(
        ...
        if note_x + 20 < inner.right() {
            for (i, line) in crate::ui::text::wrap("A group inside a card is a muted label plus indent. The focus bar stays on the control.", (inner.right() - note_x) as usize).iter().enumerate() {
