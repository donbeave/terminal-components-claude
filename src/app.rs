...
use crate::core::event::{Input, Key, Mouse, MouseKind, Outcome};
use crate::core::focus::{Focus, FocusRing};
use crate::core::hit::HitRegistry;
use crate::core::id::WidgetId;
use crate::pages::{Page, PageCtx, PageEvent, Request};
use crate::theme::{BadgeKind, Theme};
use crate::ui::ctx::{Interaction, RenderCtx, fill};
use crate::widgets::dialog::{Dialog, DialogBody};
...
impl App {
    ...
    fn open_help(&mut self) {
        ...
        d.actions.remove(0);
        d.actions[0].kind = crate::theme::ButtonKind::Secondary;
    ...
    fn draw_header(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut RenderCtx) {
        ...
        let crumb_w = crate::ui::text::width(&crumb) as u16;
        ...
        let cw = crate::ui::text::width(&cap) as u16;
    ...
    fn draw_sidebar(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut RenderCtx) {
        ...
        for (i, e) in NAV_ENTRIES.iter().enumerate() {
            ...
            buf.set_string(
                row.x + 3,
                y,
                crate::ui::text::fit(e.label, area.width.saturating_sub(4) as usize),
                label_style,
            );
    ...
    fn draw_main(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut RenderCtx) {
        ...
        let tw = crate::ui::text::width(page.title()) as u16;
        if area.width > tw + 4 {
            let blurb = crate::ui::text::truncate(page.blurb(), (area.width - tw - 3) as usize);
    ...
    fn draw_inspector(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut RenderCtx) {
        let t = self.theme;
        let panel = crate::widgets::panel::Panel::card(Some("State"));
        ...
        for (i, (k, v)) in rows.iter().enumerate() {
            ...
            buf.set_string(
                inner.x + 12,
                y,
                crate::ui::text::truncate(v, inner.width.saturating_sub(12) as usize),
                vs,
            );
        }
        let _ = ctx;
    ...
    fn draw_footer(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut RenderCtx) {
        ...
        for (k, v) in &hints {
            let kw = crate::ui::text::width(k) as u16;
            let w = kw + 1 + crate::ui::text::width(v) as u16 + 2;
        ...
        if let Some((s, _)) = &self.status {
            let w = crate::ui::text::width(s) as u16;
            if area.right() > w + 1 {
