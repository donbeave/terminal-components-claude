...
pub fn preview_sql(
    ...
    let pk_where = |src: usize| -> String {
        if pk.is_empty() {
            // no primary key: match on every original column (TablePro)
            return columns
                .iter()
                .enumerate()
                .map(|(ci, c)| {
                    ...
                    match v {
                        ...
                        other => {
                            format!("{} = {}", c.0, sql_literal(&crate::tabs::from_cell(&other)))
                        }
                    }
                })
                .collect::<Vec<_>>()
    ...
    for &r in &pending.inserted {
        ...
        for (ci, c) in columns.iter().enumerate() {
            ...
            vals.push(sql_literal(&crate::tabs::from_cell(&v)));
        }
        if names.is_empty() {
...
impl CompletionKind {
    ...
    /// TablePro base priorities (lower wins).
...
pub struct Completion {
    ...
    pub label: String,
    pub detail: String,
    /// Text to insert (may differ from label, e.g. quoting).
    pub insert: String,
    pub score: u32,
    /// Byte positions in `label` that matched, for highlighting.
...
pub fn context(src: &str, cursor: usize) -> (String, Clause, Option<String>) {
    ...
    // last keyword before the word decides the clause (within this statement)
    let stmt_start = crate::sql::statement_at(src, cursor)
        .map(|(a, _)| a)
        .unwrap_or(0);
...
mod tests {
    ...
    fn preview_sql_orders_updates_inserts_deletes() {
        use junie_tui::widgets::grid::{CellValue, ColumnSpec, DataGrid, GridRows, RowTotal};
