...
/// `! message` in the error tone, a warning as `▲ message` in the warning
/// tone — the glyph carries the meaning in monochrome, the tone in color.
/// Hints that do not fit are dropped from the right and a faint `…` marks
/// the cut.
pub fn render_toned(
    area: Rect,
    buf: &mut Buffer,
