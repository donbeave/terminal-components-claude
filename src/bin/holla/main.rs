...
//! right now. Every external system (mise, git, gh, docker, btm,
//! pg_activity, ssh, the filesystem) is a deterministic in-memory fixture;
//! every interaction is real. The real stack commands are never executed.
...
fn parse_args() -> Options {
    ...
    while let Some(a) = args.next() {
        match a.as_str() {
            ...
            "-h" | "--help" => {
                println!(
                    "holla — context-adaptive action launcher (deterministic simulation)\n\n\
...
fn main() -> std::io::Result<()> {
    ...
    // stale key presses from the launching shell must not leak into the query
