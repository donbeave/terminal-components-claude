...
use crate::domain::workspace::{AllowedRoles, DirtyExitPolicy, RoleEntry, RoleName, RoleSource, Workspace, WorkspaceId};
...
impl EdTab {
    const ALL: [EdTab; 5] = [EdTab::General, EdTab::Mounts, EdTab::Roles, EdTab::Environments, EdTab::Auth];
...
impl EditorScreen {
    pub fn new(w: &World, workspace: Option<WorkspaceId>, pending: Option<Workspace>) -> Self {
        ...
        let original = workspace.and_then(|id| w.workspace(id)).cloned().unwrap_or_else(|| {
            let mut o = pending.clone().unwrap_or_else(|| Workspace::new(0, "new", "/workspace"));
            // a freshly created Workspace starts one change away from "nothing"
            o.name = String::new();
            o
        });
        let pending = pending.or_else(|| workspace.and_then(|id| w.workspace(id)).cloned()).unwrap_or_else(|| Workspace::new(0, "new", "/workspace"));
        let roles: Vec<RoleName> = w.roles.iter().map(|r| r.name.clone()).collect();
        let cfg = ConfigTabs::new(Scope::Workspace, Doc::from_workspace(&original), Doc::from_workspace(&pending), BASE, roles);
        ...
        let mut s = Self {
            workspace,
            name: TextInput::new(NAME, "Name").required(true).help("Directory basename by default").value(&pending.name).validator(|v| {
                let v = v.trim();
                if v.is_empty() {
            ...
            dirty_policy: Select::new(DIRTY_POLICY, "On dirty exit", &["ask · show the exit dialog", "keep · preserve changes silently", "discard · drop changes silently"], policy_idx),
            original,
            pending,
            cfg,
    ...
    fn general_dirty(&self) -> bool {
        self.pending.name != self.original.name || self.pending.workdir != self.original.workdir || self.pending.keep_awake != self.original.keep_awake || self.pending.git_pull != self.original.git_pull || self.pending.dirty_policy != self.original.dirty_policy
    }
    ...
    fn build_roles(&mut self, w: &World) {
        let mut rows: Vec<RoleRow> = w.roles.iter().map(|r| RoleRow::Role(r.name.clone())).collect();
    ...
    fn validate(&mut self, cx: &mut Cx) -> bool {
        ...
        if let Some(other) = self.taken_by(&name) {
            ...
            self.set_tab(0, cx);
            cx.focus.focus(NAME);
            cx.error(format!("A workspace named {name} already exists (#{other})"));
            return false;
        }
    ...
    fn taken_by(&self, name: &str) -> Option<WorkspaceId> {
        self.taken.iter().find(|(n, _)| n == name).map(|(_, id)| *id)
    }
    ...
    fn open_preview(&mut self, w: &World, cx: &mut Cx) {
        ...
        let mut facts = vec![
            Prop::new("Workspace", if self.is_create() { format!("{} · new", self.pending.name) } else { self.pending.name.clone() }),
            Prop::new("Scope", format!("workspace config · {}", w.tilde(&format!("{}/.jackin/workspaces/{}.toml", w.home, self.pending.name)))),
            Prop::new("Changes", plural(n.max(1), "change", "changes")),
        ];
        if self.pending.name != self.original.name && !self.is_create() {
            facts.push(Prop::new("Name", format!("{} → {}", self.original.name, self.pending.name)));
        }
        if self.pending.workdir != self.original.workdir {
            facts.push(Prop::new("Working dir", format!("{} → {}", self.original.workdir, self.pending.workdir)));
        }
        facts.extend(self.cfg.summary_facts());
        if self.pending.roles != self.original.roles {
            facts.push(Prop::new("Roles", format!("{} · default {}", self.pending.roles.summary().to_lowercase(), self.pending.roles.default.clone().unwrap_or("none".into()))));
        }
        for b in &blockers {
        ...
        if self.is_create() {
            code.insert(0, format!("+ workspace {}  {}", self.pending.name, self.pending.workdir));
        }
        if self.pending.keep_awake != self.original.keep_awake {
            code.push(format!("~ keep_awake {} → {}", self.original.keep_awake, self.pending.keep_awake));
        }
        if self.pending.git_pull != self.original.git_pull {
            code.push(format!("~ git_pull {} → {}", self.original.git_pull, self.pending.git_pull));
        }
        if self.pending.dirty_policy != self.original.dirty_policy {
            code.push(format!("~ dirty_exit {} → {}", self.original.dirty_policy.label(), self.pending.dirty_policy.label()));
        }
        let confirm = if blockers.is_empty() { Button::primary(WidgetId::of("editor.preview.save"), "Save") } else { Button::primary(WidgetId::of("editor.preview.cleanup"), "Clean up & save…") };
        let title = if self.is_create() { "Create workspace" } else { "Save workspace" };
        let d = Dialog::facts(WidgetId::of("editor.preview"), title, facts, code, None, confirm);
        cx.open(Modal::Dialog(d), ModalTag::new(if blockers.is_empty() { "preview" } else { "preview.cleanup" }));
    }
    ...
    fn finish_save(&mut self, ok: bool, w: &mut World, cx: &mut Cx) {
        ...
        if !ok {
            ...
            let d = InfoDialog::new(
                ...
                &title,
                vec![
                    Prop::new("Error", "write failed: ~/.jackin/workspaces is not writable (EACCES)"),
    ...
    fn leave(&mut self, cx: &mut Cx) {
        ...
        if self.change_count() > 0 || self.is_create() {
            let d = Dialog::confirm(WidgetId::of("editor.exit"), "Unsaved changes", &format!("Save changes before leaving? {} would be lost.", plural(self.change_count().max(1), "change", "changes")), "Save").with_actions(
                vec![Button::subtle(WidgetId::of("editor.exit.cancel"), "Cancel"), Button::danger(WidgetId::of("editor.exit.discard"), "Discard"), Button::primary(WidgetId::of("editor.exit.save"), "Save")],
                Some(0),
            );
    ...
    fn render_general(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut RenderCtx, w: &World) {
        ...
        y += 4;
        buf.set_string(x, y, "Working directory *", t.secondary().bg(bg).add_modifier(Modifier::BOLD));
        y += 1;
        let wd_focused = ctx.interaction.focused(WORKDIR);
        buf.set_string(x, y, fit(&truncate(&self.pending.workdir, fw.saturating_sub(14) as usize), fw.saturating_sub(14) as usize), if wd_focused { t.primary().bg(bg) } else { t.secondary().bg(bg) });
        self.workdir_button.render(Rect::new(x + fw.saturating_sub(11), y, 11, 1), buf, ctx, bg);
        y += 1;
        let hint = if self.pending.workdir != self.original.workdir { format!("Inside the Construct · was {}", self.original.workdir) } else { "Inside the Construct".into() };
        ...
        y += 2;
        self.dirty_policy.render(Rect::new(x, y, fw.min(48), 3), buf, ctx, bg);
        let _ = w;
    }
    ...
    fn render_roles(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut RenderCtx, w: &World) {
        ...
        let head = format!("Allowed roles  {}", if matches!(self.pending.roles.allowed, AllowedRoles::All) { "all".to_owned() } else { format!("{allowed_n} of {total}") });
        buf.set_string(area.x + 2, area.y, &head, t.secondary().bg(bg).add_modifier(Modifier::BOLD));
        let def = format!("default {}", self.pending.roles.default.as_deref().map(|d| format!("★ {d}")).unwrap_or("none".into()));
        buf.set_string(area.right().saturating_sub(width(&def) as u16 + 2), area.y, &def, t.faint().bg(bg));
        let body = Rect::new(area.x, area.y + 1, area.width, area.height.saturating_sub(1));
        self.role_area = body;
        ctx.control(ROLES, body, false);
        let name_w = self.role_rows.iter().map(|r| match r {
            RoleRow::Role(n) => width(n),
            RoleRow::Load => 0,
        }).max().unwrap_or(8).clamp(8, 24) as u16;
        for (i, row) in self.role_rows.iter().enumerate() {
            ...
            match row {
                RoleRow::Load => {
                    buf.set_string(rect.x + 4, y, "+ Load role…", st.fg(if s.focused { t.text_primary } else { t.text_secondary }));
                }
                RoleRow::Role(name) => {
                    ...
                    let changed = self.original.roles.allows(name) != allowed || (self.original.roles.default.as_deref() == Some(name.as_str())) != is_default;
                    buf.set_string(rect.x + 3, y, if changed { "•" } else { " " }, st.fg(t.warning));
                    buf.set_string(rect.x + 5, y, if allowed { "[✓]" } else { "[ ]" }, st.fg(if allowed { t.text_primary } else { t.text_muted }));
                    buf.set_string(rect.x + 9, y, fit(name, name_w as usize), if s.selected { st.add_modifier(Modifier::BOLD) } else { st });
                    let sx = rect.x + 10 + name_w;
                    buf.set_string(sx, y, if is_default { "★" } else { " " }, st.fg(if s.focused { t.text_primary } else { t.text_secondary }).add_modifier(if s.focused { Modifier::BOLD } else { Modifier::empty() }));
                    let (meta, tone) = match entry {
                        Some(RoleEntry { load_error: Some(e), .. }) => (format!("! load error · {e}"), t.error),
                        Some(e) if !e.in_registry => (format!("{} · not in registry", e.source.label()), t.warning),
                        Some(e) => (format!("registry · {} · {}", if e.trusted { "trusted" } else { "untrusted" }, e.description), t.text_muted),
                        None => ("not in registry".into(), t.warning),
                    };
                    let mx = sx + 3;
                    buf.set_string(mx, y, truncate(&meta, rect.right().saturating_sub(mx + 1) as usize), st.fg(tone));
                }
            }
    ...
    fn roles_key(&mut self, key: &Key, w: &mut World, cx: &mut Cx) -> Outcome {
        ...
        let Some(row) = self.role_rows.get(self.role_cursor).cloned() else { return Outcome::Ignored };
        match key.code {
            KeyCode::Up | KeyCode::Char('k') => self.role_cursor = self.role_cursor.saturating_sub(1),
            KeyCode::Down | KeyCode::Char('j') => self.role_cursor = (self.role_cursor + 1).min(n - 1),
            ...
            KeyCode::Char(' ') => {
                if let RoleRow::Role(name) = &row {
                    ...
                    self.pending.roles.allowed = if list.len() == all.len() && all.iter().all(|r| list.contains(r)) { AllowedRoles::All } else { AllowedRoles::Custom(list) };
                    cx.status(format!("{name} · {}", if self.pending.roles.allows(name) { "allowed" } else { "not allowed" }));
                }
            }
            KeyCode::Enter | KeyCode::Char('*') => match &row {
                ...
                RoleRow::Role(name) => {
                        ...
                        && e.load_error.is_some()
                    {
                        ...
                        cx.error(format!("{name} must be allowed before it can be the default"));
    ...
    fn role_items(&self, w: &World, query: &str) -> (Vec<PickerItem>, Vec<String>) {
        ...
        for r in &w.roles {
            ...
            if !q.is_empty() && !label.to_lowercase().contains(&q) && !r.description.to_lowercase().contains(&q) {
                continue;
            }
            let already = self.pending.roles.allows(&r.name) && matches!(self.pending.roles.allowed, AllowedRoles::Custom(_));
    ...
    fn open_trust_dialog(&mut self, e: &RoleEntry, cx: &mut Cx) {
        ...
        let facts = vec![Prop::new("Role", e.full_name()), Prop::new("Repository", repo)];
        let d = Dialog::facts(
            WidgetId::of("editor.trust"),
            "Trust role source",
            facts,
            vec!["Trust this role source?".into(), "Dockerfile can run during image builds.".into(), "The role can access mounted workspace files.".into()],
            None,
    ...
    fn start_role_load(&mut self, name: &str, w: &mut World, cx: &mut Cx) {
        ...
        w.schedule(1100, Msg::RoleLoaded {
            role: name.to_owned(),
            ok,
            error: if ok { None } else { Some("manifest fetch failed: 403 from github.com/acme-labs/roles-experimental".into()) },
        });
        cx.status(format!("Loading role {name}…"));
    }

    fn finish_role_load(&mut self, name: &str, ok: bool, error: Option<String>, w: &mut World, cx: &mut Cx) {
        ...
        match w.roles.iter_mut().find(|r| r.name == name) {
            Some(r) if ok => {
                ...
                self.pending.roles.allowed = if list.len() == all.len() { AllowedRoles::All } else { AllowedRoles::Custom(list) };
            ...
            Some(r) => {
                ...
                let d = InfoDialog::new(
                    ...
                    vec![Prop::new("Error", error.unwrap_or("unknown".into())), Prop::new("Source", r.source.label()), Prop::new("Next", "trust stays granted · retry from the Roles tab")],
                )
                .error()
                .width(70);
    ...
    fn open_workdir_picker(&mut self, w: &World, cx: &mut Cx) {
        ...
        for m in &self.cfg.pending.mounts {
            items.push(PickerItem {
                ...
                glyph: "▪",
                group: "mounts",
                tag: if m.destination == self.pending.workdir { Some("current") } else { None },
                matched: vec![],
                disabled: false,
            });
            ...
            if let crate::domain::workspace::MountSource::Host(src) = &m.source {
                ...
                for f in w.fs.iter().filter(|f| f.dir && f.path.starts_with(&format!("{src}/")) && !f.path[src.len() + 1..].contains('/')) {
                    ...
                    items.push(PickerItem {
                        ...
                        tag: if dest == self.pending.workdir { Some("current") } else { None },
                        matched: vec![],
                        disabled: false,
                    });
                    targets.push(dest);
...
impl EditorScreen {
    ...
    fn refresh_taken(&mut self, w: &World) {
        self.taken = w.workspaces.iter().filter(|x| Some(x.id) != self.workspace).map(|x| (x.name.clone(), x.id)).collect();
    }
}
...
impl Screen for EditorScreen {
    ...
    fn on_tick(&mut self, _w: &mut World, _cx: &mut Cx) -> Outcome {
        if self.saving || self.pending_load.is_some() { Outcome::Changed } else { Outcome::Ignored }
    }
    ...
    fn on_msg(&mut self, msg: &Msg, w: &mut World, cx: &mut Cx) -> Outcome {
        match msg {
            ...
            Msg::RoleLoaded { role, ok, error } if self.pending_load.as_deref() == Some(role.as_str()) => {
    ...
    fn on_key(&mut self, key: &Key, w: &mut World, cx: &mut Cx) -> Outcome {
        ...
        let body = match self.tab {
            EdTab::General if matches!(focus, Some(NAME) | Some(WORKDIR) | Some(KEEP_AWAKE) | Some(GIT_PULL) | Some(DIRTY_POLICY)) => self.general_key(key, w, cx),
    ...
    fn on_paste(&mut self, text: &str, _w: &mut World) -> Outcome {
        if self.name.editing { self.name.on_paste(text) } else { Outcome::Ignored }
    }
    ...
    fn on_modal(&mut self, tag: &ModalTag, result: ModalResult, w: &mut World, cx: &mut Cx) -> Outcome {
        ...
        match (tag.kind, result) {
            ...
            ("exit", ModalResult::Dialog { action: Some(1), .. }) => {
                cx.status(format!("Discarded {}", plural(self.change_count().max(1), "change", "changes")));
                cx.go(Go::Manager);
                Outcome::Changed
            }
            ...
            ("role.load", ModalResult::Picked(i)) => {
                let Some(name) = self.picker_targets.get(i).cloned() else { return Outcome::Changed };
            ...
            ("role.trust", ModalResult::Dialog { action: Some(1), .. }) => {
                let name = tag.key.clone();
                if let Some(r) = w.roles.iter_mut().find(|r| r.name == name) {
                    r.trusted = true;
                }
                if let Some(t) = w.global.trust.iter_mut().find(|t| t.source.contains("roles-experimental")) {
                    t.trusted = true;
                }
    ...
    fn hints(&self, focus: Option<WidgetId>, _w: &World) -> Vec<Hint> {
        ...
        if focus == Some(TABS) {
            ...
            match self.tab {
                EdTab::General => {
                    if focus == Some(NAME) {
                        v.push(if self.name.editing { hint("Enter", "Commit") } else { hint("Enter", "Edit") });
                ...
                EdTab::Roles => {
                    match self.role_rows.get(self.role_cursor) {
    ...
    fn strip_right(&self, w: &World) -> Vec<Segment> {
        ...
        let n = self.change_count();
        if n > 0 || self.is_create() {
            v.push(Segment::new(format!("• {}{}", plural(n.max(1), "change", "changes"), if self.is_create() { " · new" } else { "" }), Tone::Warning).priority(8));
        }
