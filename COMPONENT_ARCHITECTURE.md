...
* **The `author` surface is sufficient for example 12** — Scenario G's mechanical proof holds — and for Button and Tabs. Field/TextInput and List are "almost" (MI‑16 and F4); ScrollRegion is "almost" (F15). **Dialog-as-layer** and **any custom-family component** were **not** sufficient: F24/§26 N1 and F14 respectively. One further gap: `Ui::surface_style()` (§26). ~~and a declared `Ui::scroll_region(id, part, …)` convenience — §12.2 names it … an open item, not decided here.~~ **Struck by §35**: it was not a gap in the author surface. `ScrollRegion::new(id).draw(ui, area, &st, content_len)` is the API and is complete.
...
## §35 Adjudication — `Ui::scroll_region` is struck, and the defect class behind it <!-- amended by §35 -->
...
**`Ui::scroll_region` is struck. It does not exist and will not.** The API is `ScrollRegion::new(id).draw(ui, area, &st, content_len)`.

Four independent reasons, any one sufficient:

1. **The declared signature is unimplementable, and the implementable one is not the declared one.** §12.2 wrote `Ui::scroll_region(id, part, …)` in the *same sentence* as the constraint that makes it impossible — "the scrollbar is `Part::TRACK`/`Part::THUMB` of its container, not a separate id space" — and `ScrollRegion::PARTS` is the fixed `[TRACK, THUMB, CONTAINER]`. There is no caller-chosen part for `part` to name. Landing it would have required amending §12.2 anyway, at which point the only question left is whether the method earns its existence.
...
`xtask doc-check` **did** see `Ui::scroll_region`, matched it, found it resolved to nothing, and was told to ignore it by `xtask/doc_check_allow.txt`. **The mechanism worked and the escape hatch defeated it.** That is the finding, and it is the same shape as §33.3's `extra` hatch on the parts check.
...
`Ui::scroll_region`'s allow-list entry is deleted, which converts it from a suppressor into a live gate: any future bare mention now fails `xtask doc-check`. The `KIND` column and the three staleness rules are owed as an `xtask` change; recorded here so this does not become a fourth instance.
...
Two corollaries in the same function. §35.2's new rule *"a suppression with no hits is a failure"* **is not implemented** — `doc_check` collects the allow-list hits and only prints them, never comparing. And `doc_check_allow.txt` now carries roughly 150 **satisfied** entries sitting under a header that says entries below it must *not* resolve, plus five duplicate pairs. §35's claim that deleting the `Ui::scroll_region` entry "converts it into a live gate" is therefore false for §27–§39.
