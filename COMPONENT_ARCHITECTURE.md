...
| J4 | Info/facts dialog with copyable rows | **Library convenience** — `Dialog::facts(...)` over `Props` + a scrollable detail slot; supersedes `deleted facts-body type` and TablePro's button surgery. |
...
pub struct FieldSpec<'a> {
    ...
    pub plain:    bool,              // forwarded to Field::plain (kills the old plain-label builder)
}
impl<'a> FieldSpec<'a> {
...
| 17 | `InfoDialog` (`jackin/screens/modals.rs:1947-2280`) | **move** | `Dialog::facts(...)` + `Props` + a scrollable detail slot | `«tui»/components/dialog.rs` | Supersedes `deleted facts-body type` **and** TablePro's post-construction button surgery (J4). |
...
| `ratatui-core/layout-cache`, `palette`, `scrolling-regions` features | MOD §1.4 (§22) | `layout-cache` is a process-global LRU that perturbs §16.6's deterministic allocation counts and caches a solver §10 does not use; `palette` adds only `From<Srgb>` (there is no HSL conversion) and pulls `libm`; `scrolling-regions` affects inline viewports only | features exactly `{std, underline-color}` |
...
| R‑10 | **Colour literals are `Color::from_u32`, and only inside `theme/builtin/`.** There is no HSL conversion. | `Color::from_u32` (`style/color.rs:133-138`) |
...
* **The `author` surface is sufficient for example 12** — Scenario G's mechanical proof holds — and for Button and Tabs. Field/TextInput and List are "almost" (MI‑16 and F4); ScrollRegion is "almost" (F15). **Dialog-as-layer** and **any custom-family component** were **not** sufficient: F24/§26 N1 and F14 respectively. One further gap: `Ui::surface_style()` (§26). ~~and a declared Ui scroll convenience — §12.2 names it … an open item, not decided here.~~ **Struck by §35**: it was not a gap in the author surface. `ScrollRegion::new(id).draw(ui, area, &st, content_len)` is the API and is complete.
...
re-create the legacy swallow by accident. ~~`DialogCase` gains `TRAPS_FOCUS`. **OWED, not done (§40):** at the time of writing `DialogCase` still declares `ACTIVATES | FOCUSABLE | OVERLAY`, the driver contains none of the three modifications, and case 14's trap half has therefore **still never executed**. This section asserted an executable artefact in the present indicative — §35.2's own named defect class, committed in the same session that named it.~~ **Historical wording retained; superseded by the implementation evidence below.** <!-- amended by integration checkpoint -->

**Implementation evidence (2026-09-05, current at `5d17cc0`).** `DialogCase::caps()` now declares `ACTIVATES | FOCUSABLE | OVERLAY | TRAPS_FOCUS`. Case 14's `focus_trap_and_restore` driver now enforces the modal/capability correspondence in both directions, rejects an empty trap, checks forward and reverse `Tab` wrapping, and checks that a zero-area modal still traps. The runtime now produces `DismissReason::FocusOut` for a popover that loses focus and suppresses focus restoration on that path; targeted coverage exists in `layer::focus_out_dismisses_a_popover_but_never_a_modal`, `form::open_select_popover_dismisses_on_focus_out_and_esc_closes_only_the_popover`, and conformance case 14. This records implemented evidence only; it is not a gate result or a blessing authorization.
...
- **`xtask doc-check` does not compile §17's examples.** It never has. It is a *name-existence* checker: it matches `` a type/member pattern `` in prose and `use` lists and asks whether the name exists in a `syn` scan. It never parses a signature. The artefacts that compile are `crates/tui/examples/*.rs`, which are different files from §17's inline blocks. **Proof:** §17.0 A2 declares `register_editor(&mut self, id: Id, area: Rect)` where the real signature takes four arguments, and declares `styled_parts` returning a five-tuple where it returns `&[(Id, Part)]` — and doc-check is green on both, because the names resolve.
...
The old Ui scroll-convenience allow-list entry is deleted, which converts it from a suppressor into a live gate: any future bare mention now fails `xtask doc-check`. The `KIND` column and the three staleness rules are owed as an `xtask` change; recorded here so this does not become a fourth instance.
