...
| API-4 | Design inconsistency / P2 | `docs/design/DESIGN.md:1250` says no context menu or diff viewer. Menu catalogue exists at L966, `widgets/menu.rs:72` implements ContextMenu; DiffView exists. | Contract contradicts implementation, encouraging duplicate widgets or incorrect audits. Correct absence statement and document DiffView. README architecture list also omits later modules. | Low: reconcile catalogue against exports, pages and actual apps. |
