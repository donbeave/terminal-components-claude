# holla vs holla-fable comparison ledger

Branch refs (verified 2026-09-14):

| Ref | SHA |
| --- | --- |
| `holla` | `772b7a225baf56ab130c857cb7a49dddc7bae34a` |
| `holla-fable` | `3dbc73e253a1c4aebbe9cccbfc6d5c5e3ff8d73e` |
| merge-base | `3dbc73e2` (= `holla-fable`; `holla` is 38 commits ahead) |

Relationship: **not a merge task**. `holla-fable` is the historical reference; `holla` already contains every commit from `holla-fable` plus additive work. Comparison is semantic regression audit + selective restoration.
...
| Application binaries | **holla strictly better** on integration (scroll, determinism, responsive UX, coverage tests) except **CLI help richness** (ported in this pass) |
| Test/snapshot infrastructure | **holla strictly better** (tuisnap harness, grouped snapshots, `app_tests_coverage`, nextest) |
| Documentation | **Reorganized on holla**; knowledge preserved under `docs/`; capture workflow refs updated to tuisnap paths |
| Legacy `shots/` corpus | **Partially superseded** by `snapshots/`; some frozen frames documented in `docs/baseline/tuisnap-coverage.md` |

## Decision ledger

| Area | holla-fable | holla before | Decision | Final state | Evidence |
| --- | --- | --- | --- | --- | --- |
| Component library | Baseline widgets/runtime | Same code | **HOLLA_BETTER (tie)** | Unchanged | `git diff holla-fable..holla -- src/lib.rs src/widgets src/core` empty |
...
## 38-commit themes (holla-fable..holla)
...
- App integration: holla equal or better
- CLI help: restored on holla
- Visual baseline: holla superset with documented frozen legacy frames

Outstanding **product parity** gaps (HP01–HP23 vs legacy real Holla) are documented in parity matrices and are **future product work**, not regressions introduced between `holla-fable` and `holla`.
