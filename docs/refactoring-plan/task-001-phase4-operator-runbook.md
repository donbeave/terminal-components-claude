...
| Rule | Detail |
| --- | --- |
| **Do not merge to `main`** | All production edits stay on `task-001-bootstrap` in the architectural worktree. No push, merge, or integration to `refs/heads/main`. |
...
| **Do not use wrapper for host matrix** | Qualification must target the built Mach-O at `target/debug/tc-proof-host`, not the shell wrapper at `tools/refactor-proof/bin/tc-proof-host` (install byte-equality fails on wrapper — see qualification report §4a). |
...
- Branch `task-001-bootstrap` at or ahead of `9e1847fc`
...
All candidate edits and builds happen in the architectural worktree, not on `prep-wave1-verify`.
...
if [ -d "$TC_WORKTREE/.git" ] || [ -f "$TC_WORKTREE/.git" ]; then
  ...
  git -C "$TC_WORKTREE" checkout task-001-bootstrap
else
  git fetch origin main
  git worktree add -B task-001-bootstrap "$TC_WORKTREE" "$ARCH_MAIN"
fi

git -C "$TC_WORKTREE" rev-parse HEAD
git -C "$TC_WORKTREE" rev-parse HEAD^   # expect $ARCH_MAIN when at Phase 3 tip
git -C "$TC_WORKTREE" branch --show-current   # expect task-001-bootstrap
```
...
| --- | --- | --- | --- |
| OB-002 | Operator | Confirm worktree @ EV-002; branch `task-001-bootstrap` | **EV-002** |
...
From the candidate worktree, with explicit config, base, and progress:
