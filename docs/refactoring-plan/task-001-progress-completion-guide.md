...
**Worktree:** `task-001-bootstrap` @ `cf2e79a068518e229751f82b635832ecaba8ae4d`  
**Sandbox:** `.worktrees/main/.qual/task-001-progress-sandbox/` (gitignored scratch — never commit)  
...
3. At **3.1**, run `taskfmt verify --progress ""` from `/work` until exit 0 and last line `DONE` (AGENTS.md step 7), **then** append `DONE | 3.1`.
...
Complete events through **3.1** per §5, provision `/task`, `/work`, `/proof/bootstrap` mounts, then:
