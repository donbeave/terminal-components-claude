...
> **Historical snapshot:** Measured on retired `.worktrees/main` @ `task-001-bootstrap`. Campaign operators use `.worktrees/campaign` / `refactor/holla-parity` — see [path-contract.md](path-contract.md).
...
| Host matrix (`--host` via `bin/tc-proof-host` post-sync) | **PASS** | 63 | 0 |
| `cargo nextest run -p refactor-proof` | **PASS** | 17 | 0 |
