...
- Tui-snap upstream main is `0a2e490802b7b048cd96349c6af860f8a3a05c3d` (2026-09-15). [PR #1](https://github.com/donbeave/tui-snap/pull/1) merged at reviewed head `883d03f19d890bbbf27468798db78b04e85297ac`; terminal-components pins the current tip in `Cargo.toml`.
...
| Catalog argv smoke (`validate-plan.py` `catalog_argv_smoke`) | PASS with zero errors: 479 `/proof/bin/tc-proof` argv references across 73 packages have required check fields, known operations (`preflight`, `required`, `oracle`, `capture`, `compare`, `account-tests`, `architecture`, `close`), non-empty argv segments, and per-check `--context /run/tc-proof/contexts/CHK-NNN.json` paths matching each check id. Oracle/capture checks require `--namespace` / `--lane` respectively. Does not invoke the absent binary. |
