...
**Worktree branch:** `refactor/holla-parity` (campaign; seeded from historical `task-001-bootstrap`)  
**Worktree path:** `.worktrees/campaign` (repo-relative) — see [`path-contract.md`](path-contract.md) § Worktree  
**Historical note:** Phases 0–3 below were executed on retired branch `task-001-bootstrap` @ `.worktrees/main`; operators now use [`task-001-phase4-operator-runbook.md`](task-001-phase4-operator-runbook.md) on `.worktrees/campaign`.
...
1. **Confirm worktree:** `cd /Users/donbeave/Projects/terminal-components-claude/.worktrees/campaign` — branch `refactor/holla-parity` (record HEAD at dispatch).
