...
**Catalog commit:** `a80656f2` on branch `prep-wave1-verify` (verify with `git rev-parse HEAD`)  
**Parent planning tip:** `1a873cff` → Wave 2 bootstrap work (IW-02/03 + TASK-001 plan) on this branch  
**TASK-001 worktree tip:** `c9c6a3ba` on branch `task-001-bootstrap` (verify with `git ls-remote origin task-001-bootstrap`)  
...
| Prerequisite | Owner |
| --- | --- |
| `tc-proof-host` implementation + qualification | TASK-001 worktree @ `c9c6a3ba` — Phase 3 **complete** (CHK-004 141/141, host matrix 63/63, nextest 17/17, context-check subprocess dispatch, architecture exemption); committed `bin/*` are shell wrappers @ `c9c6a3ba` — operators run `sync-binaries.sh` before host qualification; review fixes @ `05b20ad4`/`cd910c7b`/`3ed51570`/`c9c6a3ba`; IW-03 operator checklist required before production receipt — see [`task-001-bootstrap-plan.md`](task-001-bootstrap-plan.md) |
...
| Mechanical validator | `validate-plan.py --summary` error_count 0 @ catalog commit `a80656f2` |
