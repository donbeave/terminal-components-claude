...
| `⚠` | unavailable, blocked | discovery failures, blocked rows — the reason text must say why |
...
- **Exception — the tonal entity strip**: when the strip lists *entities with
  ...
  tone on the glyph, hit ids per entity, overflow breaks right-to-left. Tabs
