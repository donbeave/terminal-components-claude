...
the latest stable Lychee release. GitHub Actions workflows are generated by
`velnor-workflow` into [`.github/workflows/`](.github/workflows/); the current
generated `ci-extra-gates.yml` includes a Lychee job. Enumerate the tracked
Markdown inputs with Git and run the native local check below. The current
qualification is Lychee `0.24.2`; re-qualify when the version changes.
All external GitHub Actions in the generated workflows are pinned to full
commit SHAs. An action update requires verifying the upstream release or ref,
changing [`.github-gen/velnor-workflow.toml`](.github-gen/velnor-workflow.toml)
(or the generator), regenerating, and re-running workflow syntax validation.
