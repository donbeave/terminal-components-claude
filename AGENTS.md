...
`velnor-workflow` into [`.github/workflows/`](.github/workflows/) and do not
include a Lychee job. Enumerate the tracked Markdown inputs with Git and run
the native local check below. The current qualification is Lychee `0.24.2`;
re-qualify when the version changes.
