...
if [[ -L "$BINARY" || -L "$RECEIPT" || -e "$BINARY" || -e "$RECEIPT" ]]; then
  [[ -f "$BINARY" && ! -L "$BINARY" && -f "$RECEIPT" && ! -L "$RECEIPT" ]] \
    || { echo "campaign-build-proof: stale or linked proof target artifacts" >&2; exit 1; }
  validate_receipt
fi
...
[[ -d "$DEBUG_DIR" && ! -L "$DEBUG_DIR" ]] \
  || { echo "campaign-build-proof: native debug target directory missing or linked: $DEBUG_DIR" >&2; exit 1; }
[[ -f "$BINARY" && ! -L "$BINARY" && -x "$BINARY" ]] \
  || { echo "campaign-build-proof: native comparator missing: $BINARY" >&2; exit 1; }
[[ ! -L "$RECEIPT" ]] \
  || { echo "campaign-build-proof: native build receipt must not be a symlink: $RECEIPT" >&2; exit 1; }
