...
visual_baseline -- --ignored --skip report` and review it via `tuisnap
report` / `tuisnap accept` (see `docs/baseline/tuisnap-coverage.md`).
...
cell-exact and pixel-exact. The runner is the Rust integration suite
`tests/visual_baseline/` (tuisnap as a library): one `#[test] #[ignore]` per
capture — 367 ported from the retired `tools/tuisnap_baseline.sh` plus a
15-capture pointer group (hover, drag-select, wheel scroll-fade, resize
sequences) — so default `cargo test` compiles the suite but runs no PTYs
(the matrix and its rationale live in
[docs/baseline/tuisnap-coverage.md](docs/baseline/tuisnap-coverage.md)).

```sh
cargo test --test visual_baseline -- --ignored --skip report  # regenerate the matrix
...
failed); review `shots/tuisnap/report.html`, approve, then re-verify.
...
tuisnap matrix; the mouse lanes (hover/drag/wheel-fade) and resize sequences
are regenerated by the suite's pointer group, so only wall-clock motion
phases and a few fragile focus walks survive solely as frozen frames.
