...
record_provenance() {
  ...
  local theme=${15} color=${16} stderr_path=${17}
  ...
  python3 - "$CAPTURE_MANIFEST" "$name" "$columns" "$rows" \
    ...
    "$run_id" "$revision" "$theme" "$color" "$stderr_path" <<'PY'
import hashlib
import json
import os
import sys
...
case "$cmd" in
  ...
  shot)
    ...
    record_provenance "$name" "$cols" "$rows" "$capture_status" "$exit_status" \
      ...
      "$requested_columns" "$requested_rows" "$run_id" "$revision" "$theme" \
      "$color" "$STDERR_FILE"
    if [[ "$capture_status" == failed ]]; then
      exit 1
    fi
    echo "$html_path ($cols x $rows; theme=$theme color=$color exit=$exit_status; provenance=$CAPTURE_MANIFEST)"
    ;;
  resize)
    require_session
