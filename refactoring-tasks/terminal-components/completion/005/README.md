...
schema: task/v5
id: TASK-005
title: "Capture all immutable TablePro oracle scenarios"
kind: test
---

# TASK-005 — Capture all immutable TablePro oracle scenarios

## Goal

Every TablePro oracle application route and TP scenario has complete repeatable action-reached direct/PTY evidence.
...
This is a later execution task. The current planning goal only creates this immutable package. UI authority is `02f5294bfdbf38004cc49130d0aff1d01f31434c`; architecture starts from `7b27732a8c3c131760ec3438f641cb3c11343a42`. The accepted producer product is `oracle-tablepro`. Task dependencies are `TASK-071`, `TASK-072`. TASK-001 and TASK-070 stay retired fail-closed and cannot supply accepted producer receipts. Former host/runner/lifecycle obligations now live in `scripts/campaign-build-proof.sh`, [`path-contract.md`](../../../../docs/refactoring-plan/path-contract.md), [`proof-contract.md`](../../../../docs/refactoring-plan/proof-contract.md), and `tools/refactor-proof/{src,runner,bin}`. Dependency status alone never proves integrated source ancestry or trusted product acceptance. Follow the subagent-only host-local workflow in proof-contract.md; use standalone taskfmt only for this package's lint and verify evidence.
...
- `tools/refactor-proof-adapters/tablepro` for this task's stated product and directly necessary tests.
- Independently inspectable oracle-tablepro evidence and exact provenance; write generated run outputs only to host-assigned directories.
...
- **R-001 (MUST):** Expand and independently qualify every app-specific flow contribution and source-derived direct seed under `trusted/app-flow-contribution-contract.md`, preserving every intact parent event/assertion/frame and the separate seeded-versus-PTY lane classification. Satisfy every exact clause mapped to R-001 in `trusted/source-obligations.tsv` together with the bounded outcome below. Expand TP-001–TP-080, including all ALL sizes and truecolor/256/16/mono/NO_COLOR profiles, Connections/form actions, Explorer/tabs/drawer, typed Grid/filter/Structure, SQL editor/results/Explain, six safety modes, history and every real overlay. Expand presets C/W/L/T/Q/QL by their actual startup/input sequences and preserve safety mode within Q actions.
- **R-002 (MUST):** Satisfy every exact source clause mapped to R-002 in the protected source obligations. Use the original in-memory connection/catalog/history/coordinator fixture and production handlers/rendering. Record alias-resolved control IDs and oracle coordinates, Ctrl+L replacement behavior, exact completion/query/error ticks, statement-at-cursor semantics, and 101/102 drawer threshold. Qualify Instant seam for140ms flash and five-second status without moving conditions or erasing text.
- **R-003 (MUST):** Preserve every source-qualified assertion and test mapped to R-003 in the protected source obligations. Execute all original35tests and exact SQL/model/application assertions. Capture every press checkpoint and intermediate result, target identity, pending SQL, selected mode, overlay/focus/cursor/scroll and ignored action. Repeat flattened traces in isolated fresh processes/worlds; keep private semantic and terminal-observable proofs separate.
- **R-004 (MUST NOT):** Violate any rejection, deferral, non-goal or trust rule mapped to R-004 in the protected source obligations. Never use main Surface enum labels or static self-digests as mounted-overlay proof, insert live DB/SSH/keychain/clipboard access, retarget coordinates from candidate geometry, replace SQL CodeEditor with TextInput, or capture expected frames from current main.
...
When the oracle-tablepro producer executes the exact work in R-001
...
```gherkin
Given the completed oracle-tablepro implementation
...
- **D-002:** This task produces `oracle-tablepro`. Only verifier and reviewer subagents can approve evidence after independent checks; the coordinator cannot edit receipts or task metadata.
...
- [ ] **2** Produce the bounded evidence product.
    - [ ] **2.1** Expand exact TP fixtures, modes, palettes and source-resolved actions. (`R-001`, `AC-001`, `CHK-004`)
    - [ ] **2.2** Qualify production observation and the narrow clock adapter. (`R-002`, `AC-002`, `CHK-006`)
    - [ ] **2.3** Capture/replay every direct and PTY-required checkpoint twice. (`R-003`, `AC-003`, `CHK-005`)
    - [ ] **2.4** Validate complete route/overlay membership and submit oracle-tablepro for sealing. (`R-001`, `AC-001`, `CHK-004`)
