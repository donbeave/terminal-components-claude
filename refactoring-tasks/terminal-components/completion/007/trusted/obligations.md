# Protected task inputs for TASK-007

Product: `test-inventory`.

Use independently qualified preparation accounting, not this task's future accepted inventory or TASK-008 disposition receipt. Discover every inline/external/generated/macro test across all crate/app source trees and required profiles; reconcile source-qualified assertion spans with actual compiled listings/execution. The protected inline-test-source-scope.md location seed does not replace completeness proof. Emit preserve or exact oracle-conflict dispositions for all assertions without editing them.

Reconcile the 3,211 previously inventoried historical test identities with all newly pinned oracle tests and the complete 620-row historical union. Record source commit, package, binary/test target, fully qualified test name, feature/toolchain profile and actual execution command. Preserve independently meaningful requirements even when source rows are duplicate-equivalent. Generate exact required inventory and fail absent targets, zero matches, missing results or unapproved name relocation.

Reuse tools/test-inventory and the qualified tc-proof account-tests protocol; source parsing discovers identities but never substitutes for execution. Separate accepted primary trybuild stderr owner from MSRV compilation/behavior. Keep historical archive provenance and test relocation identities explicit; counts alone are insufficient.

Exercise strict inventory parsing, missing/duplicate/renamed target rejection, filtered/ignored/no-fail-fast omissions, stale profile and empty allowlist failures. Preserve exact named allocation/workload tests and original assertions; document main-versus-oracle product conflicts for TASK-008 rather than changing their expectations here.

Source requirements:

- [docs/refactoring-plan/execution-readiness-report.md](../../../../../docs/refactoring-plan/execution-readiness-report.md).
- [docs/refactoring-plan/historical-obligations-canonical.tsv](../../../../../docs/refactoring-plan/historical-obligations-canonical.tsv).
- [docs/refactoring-plan/architecture.md](../../../../../docs/refactoring-plan/architecture.md).
- [tools/test-inventory/README.md](../../../../../tools/test-inventory/README.md).

The host binds the source documents, canonical historical union and scenario files named above into the immutable campaign input manifest. Their exact scenario expansion algorithms and source-qualified tests are required membership, not a best-effort glob. Rejected/deferred source clauses retain their disposition. A task-local candidate manifest cannot override this document or the protected host context.
