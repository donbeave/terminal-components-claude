...
schema: task/v5
id: TASK-017
title: "Preserve runtime layer placement capture and restoration"
kind: bugfix
---

# TASK-017 — Preserve runtime layer placement capture and restoration

## Goal

Runtime layers preserve anchor, modal ownership and restoration independently of concrete wrappers.
...
- `crates/tui/src/layer.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/ui/layer_buf.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/runtime.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_017.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** One resolver clamps/flips anchors and distinguishes Fill from Fixed zero. Test tiny/empty/offscreen anchors, changed body size/theme, reverse draw order, undrawn modal and duplicate layer. Modal traps and restores even empty; popover focus-out dismisses without restoring opener. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Layer lifecycle owns z-order/barriers; only size/anchor may change while open and no-op setters do not repaint. Keep capture origin/publication semantics; no Form/Dialog dependency or app backdrop loop. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Nested editor Esc precedes outer dismissal; runtime modal-first paste and outside/secondary clicks never leak; source-qualified application paste commands follow the fixed contract. Resize/reparent and opener removal yield exact focus/cursor and dimmed cells. Empty body remains clipped with single invocation; malicious callbacks cannot escape layer clip. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed layers-popups oracle cases and the exact task history clauses
...
```gherkin
Given the live layers-popups implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane layers-popups actions and checkpoints
...
- **D-001:** Layer lifecycle owns z-order/barriers; only size/anchor may change while open and no-op setters do not repaint. Keep capture origin/publication semantics; no Form/Dialog dependency or app backdrop loop.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_017 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
