...
schema: task/v5
id: TASK-015
title: "Complete borrowed row part and column overrides"
kind: bugfix
---

# TASK-015 — Complete borrowed row part and column overrides

## Goal

Borrowed row and column overrides paint their declared parts without losing stable identity. The same shared TASK-013 streaming projection preserves graphemes across one Display invocation's formatter fragments, with honest cold/warm scratch and output allocation proof.
...
- `crates/tui/src/collection` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_015.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** Thread borrowed explicit part overrides through RowUi::part, ColumnsUi and CellUi, not only label_patch. Preserve component container/label forwarding while explicit row/cell/custom-part patches override only their intended cells. Distinguish Clear reserved blank from Inherit, label base inheritance and glyph roles. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Caller keyed sources own data; collection caches own only derived identity/projection. Keep row-author provenance distinct from same-ID component composition through callbacks; no fabricated owner ID. No Display/Clone/static requirement for borrowed renderers or full-dataset clone in measured work. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Sentinel actual-cell tests cross inherited/family/variant/subtree/instance/part/cell/slot precedence, siblings, clipped columns and mono. Reorder/insert/delete duplicate labels and mutate revision between publication/input. Test non-Clone borrowed model, all column limits and visible-only callback counts at 100k rows. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed collection-core and identity oracle cases and the exact task history clauses
...
```gherkin
Given the live collection-core and identity implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane collection-core and identity actions and checkpoints
...
- **D-001:** Caller keyed sources own data; collection caches own only derived identity/projection. Keep row-author provenance distinct from same-ID component composition through callbacks; no fabricated owner ID. No Display/Clone/static requirement for borrowed renderers or full-dataset clone in measured work.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_015 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
