...
schema: task/v5
id: TASK-002
title: "Capture all immutable Showcase oracle scenarios"
kind: test
---

# TASK-002 — Capture all immutable Showcase oracle scenarios

## Goal

All 23 Showcase pages and every SC scenario have independently repeated, complete oracle direct/PTY action and frame evidence from the pinned source.
...
This is a later execution task. The current planning goal only creates this immutable package. UI authority is `02f5294bfdbf38004cc49130d0aff1d01f31434c`; architecture starts from `7b27732a8c3c131760ec3438f641cb3c11343a42`. The accepted producer product is `oracle-showcase`. Task dependencies are `TASK-071`, `TASK-072`. TASK-001 and TASK-070 stay retired fail-closed and cannot supply accepted producer receipts. Former host/runner/lifecycle obligations now live in `scripts/campaign-build-proof.sh`, [`path-contract.md`](../../../../docs/refactoring-plan/path-contract.md), [`proof-contract.md`](../../../../docs/refactoring-plan/proof-contract.md), and `tools/refactor-proof/{src,runner,bin}`. Dependency status alone never proves integrated source ancestry or trusted product acceptance. Follow the subagent-only host-local workflow in proof-contract.md; use standalone taskfmt only for this package's lint and verify evidence.
...
- `tools/refactor-proof-adapters/showcase` for this task's stated product and directly necessary tests.
- Independently inspectable oracle-showcase evidence and exact provenance; write generated run outputs only to host-assigned directories.
...
- **R-001 (MUST):** Satisfy every exact clause mapped to R-001 in `trusted/source-obligations.tsv` together with the bounded outcome below. Implement only the Showcase reference observation/clock adapter and source-only scenario expansion. Materialize every SC row and its finite size/color/focus/resize axis, including 368 default page frames, every live and disabled hit, Diff, General/Members/Environment settings, six Inputs controls, customer Grid pending/edit/SQL and timed form/button/output flows. Record exact coordinates, key counts, ordered checkpoints and semantic identities; candidate-derived selectors are forbidden.
- **R-002 (MUST):** Satisfy every exact source clause mapped to R-002 in the protected source obligations. Drive the original production App::handle/render and actual CLI --page/--color routes. Keep source-owned simulation ticks distinct from logical Instant deadlines. Prove clock adaptation at busy >=2200 ms, submit >1800 ms, flash >=140 ms and status >4000 ms without changing conditions or product state. Retain exact 31/32-row compact navigation, 99/100 inspector and 109/110 sidebar thresholds and minimum 72x20.
- **R-003 (MUST):** Preserve every source-qualified assertion and test mapped to R-003 in the protected source obligations. Run all original Showcase test identities from pinned source; preserve their assertions, instrument only allowed time/observation seams and show reverse-patch original-blob equality. Capture initial, every intermediate, press-before-release and ignored-input checkpoints. Replay every flattened action program afresh and require complete cell/cursor/state equality. Distinguish direct private semantics from terminal-observable PTY assertions.
- **R-004 (MUST NOT):** Violate any rejection, deferral, non-goal or trust rule mapped to R-004 in the protected source obligations. Never capture expected frames from main, fabricate live controls with inert Ui::reference, substitute the old 22-page 794b095 matrix, exclude sidebar/cursor, bless snapshots, mask transient regions, or alter shared comparator/membership policy.
...
When the oracle-showcase producer executes the exact work in R-001
...
```gherkin
Given the completed oracle-showcase implementation
...
- **D-002:** This task produces `oracle-showcase`. Only verifier and reviewer subagents can approve evidence after independent checks; the coordinator cannot edit receipts or task metadata.
...
- [ ] **2** Produce the bounded evidence product.
    - [ ] **2.1** Compile the SC source-only expansion with exact pinned fixture/test sources. (`R-001`, `AC-001`, `CHK-004`)
    - [ ] **2.2** Qualify observation and logical-time adapters against untouched source. (`R-002`, `AC-002`, `CHK-006`)
    - [ ] **2.3** Capture every direct/PTY-required row twice in isolated runs. (`R-003`, `AC-003`, `CHK-005`)
    - [ ] **2.4** Validate exact membership and submit oracle-showcase for independent host sealing. (`R-001`, `AC-001`, `CHK-004`)
