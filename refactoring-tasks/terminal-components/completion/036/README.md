...
schema: task/v5
id: TASK-036
title: "Restore Showcase Diff editor and output journeys"
kind: refactor
---

# TASK-036 — Restore Showcase Diff editor and output journeys
...
- **P-001:** Accepted receipts for `terminal-components/completion/035`, `terminal-components/completion/025`, `terminal-components/completion/026`, `terminal-components/completion/029` exist, their accepted source commits are ancestors of the integrated parent, and the whole sealed baseline/test-disposition products are present.
...
- Restore Diff unified/review/empty modes and narrow review fallback, original Unicode press anchor and copy; CodeEditor completion/find/diagnostics/run; prose versus follow-tail logs; independent scroll owners/thumb offsets; terminal seven-stage success/failure rail and split; deterministic progress pause/restart; and Task runner queued/running/max-two/failure/cancel flows. Preserve exact Tick-driven domain sequences, not an elapsed-time approximation.
- Exact owned IDs: `APP:SC-BASE-panels`, `APP:SC-BASE-progress`, `APP:SC-BASE-scrolling`, `APP:SC-BASE-terminal`, `APP:SC-BASE-editor`, `APP:SC-BASE-diff`, `APP:SC-BASE-taskrunner`, `APP:SC-PANEL-SCROLL`, `APP:SC-PROGRESS-RUN`, `APP:SC-SCROLL-OWNERS`, `APP:SC-SCROLL-THUMB`, `APP:SC-TERMINAL-RUN`, `APP:SC-TERMINAL-SELECT`, `APP:SC-EDITOR-EDIT`, `APP:SC-EDITOR-COMPLETE`, `APP:SC-EDITOR-RUN`, `APP:SC-DIFF-MODES`, `APP:SC-DIFF-SELECT`, `APP:SC-TASK-RUN`, `APP:SC-TASK-SCROLL`.
...
- **R-001 (MUST):** Restore Diff unified/review/empty modes and narrow review fallback, original Unicode press anchor and copy; CodeEditor completion/find/diagnostics/run; prose versus follow-tail logs; independent scroll owners/thumb offsets; terminal seven-stage success/failure rail and split; deterministic progress pause/restart; and Task runner queued/running/max-two/failure/cancel flows. Preserve exact Tick-driven domain sequences, not an elapsed-time approximation. Every exact row and checkpoint obligation in `trusted/obligations.md` must compare equal against the sealed oracle in its declared direct and PTY lanes; no text-only, initial-frame-only or retargeted-coordinate substitute is acceptable.
- **R-002 (MUST):** Use shared DiffView, CodeEditor, TextViewport, ScrollRegion, Split, Steps, Progress and runtime layers. No duplicated output selection, scrollbar/fade, diagnostic painter or per-app timer mechanism. Application domain pipeline and styled output content remain application-owned.
...
- **D-001:** Use shared DiffView, CodeEditor, TextViewport, ScrollRegion, Split, Steps, Progress and runtime layers. No duplicated output selection, scrollbar/fade, diagnostic painter or per-app timer mechanism. Application domain pipeline and styled output content remain application-owned.
