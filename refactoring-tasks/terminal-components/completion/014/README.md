...
schema: task/v5
id: TASK-014
title: "Restore shared scroll boundaries thumb capture and fades"
kind: bugfix
---

# TASK-014 — Restore shared scroll boundaries thumb capture and fades

## Goal

One scroll and fade primitive preserves exact hidden-edge cells and routing.
...
- `crates/tui/src/scroll.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/scroll_region.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/ui/paint.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/author.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_014.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** Implement oracle src/ui/fade.rs arithmetic once: height below four none; four through eleven one outer row at 55% contrast; twelve and above add inner row at 80%. Majority-background ties, componentwise rounding, reversed/different-background exclusion, hardware-cursor/current protected rows and non-RGB outer DIM match oracle. Exclude headers/footers/bar; paint after content and before bar. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Compose ScrollRegion directly, never revive Ui::scroll_region or collection-to-component inversion. Preserve one thumb track/grab and nearest eligible owner; boundary wheel consumes without chaining, repaint or focus shift. Hidden bars reserve no column and expose no pointer part while wheel/reveal stay active. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Test heights 3/4/11/12, fits/top/middle/bottom, resize thresholds, both themes/all capabilities, selected/hovered/marked protected cells and clipping. Hide during capture, scroll full endpoints, retain grab offset, and reject a per-app duplicate or fade changing glyph/hit/selection geometry. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed scroll-state-region oracle cases and the exact task history clauses
...
```gherkin
Given the live scroll-state-region implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane scroll-state-region actions and checkpoints
...
- **D-001:** Compose ScrollRegion directly, never revive Ui::scroll_region or collection-to-component inversion. Preserve one thumb track/grab and nearest eligible owner; boundary wheel consumes without chaining, repaint or focus shift. Hidden bars reserve no column and expose no pointer part while wheel/reveal stay active.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_014 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
