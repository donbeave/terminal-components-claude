...
schema: task/v5
id: TASK-030
title: "Preserve effective help metadata and Wizard state"
kind: bugfix
---

# TASK-030 — Preserve effective help metadata and Wizard state

## Goal

Help and Wizard preserve effective bindings and caller-owned step state.
...
- `crates/tui/src/components/help.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/wizard.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_030.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** HelpOverlay renders exact grouping, columns, scrolling and dismissal from the same effective owner/action resolver as menu and hints. Wizard stable keyed enabled steps preserve back/forward/rewind/completion data/focus across dynamic eligibility. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** No help-only decorative bindings, parallel keymap, domain jobs or redesigned wizard chrome. Retain caller-owned state and runtime layers; repeated draw cannot navigate or mutate completion. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Test full/narrow columns, remap/remove/hidden actions, nested modal help, disabled/vanished step, back then forward retaining draft and failed completion. Exact key/menu/help equivalence and restored focus remain visible in production frames. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed help-overlay and wizard oracle cases and the exact task history clauses
...
```gherkin
Given the live help-overlay and wizard implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane help-overlay and wizard actions and checkpoints
...
- **D-001:** No help-only decorative bindings, parallel keymap, domain jobs or redesigned wizard chrome. Retain caller-owned state and runtime layers; repeated draw cannot navigate or mutate completion.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_030 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
