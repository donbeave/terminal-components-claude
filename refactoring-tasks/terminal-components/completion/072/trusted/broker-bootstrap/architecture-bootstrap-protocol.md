...
The protected build host supplies rustup for explicit toolchain resolution. Freeze its selected toolchain before entering any source-copy directory, resolve both actual Cargo and rustc through that selection, and invoke those absolute executable paths. A PATH MBX/rustup launcher digest is not Cargo identity. Bind executable bytes, verbose versions, paths and toolchain; revalidate before and after compiler/metadata invocation. Cargo receives the recorded RUSTC with compiler wrappers disabled. A changed selected toolchain, path, executable or version fails, even if compilation otherwise succeeds. This establishes executable identity, not an assertion that every operating-system dynamic library is independently hashed. The preparation-only compiler-identity-selftest.py exercises this shared boundary; its diagnostic success is not a TC product receipt.
...
## Separate doc/API/dependency acceptance

TASK072 CHK-006 additionally invokes `architecture-bootstrap-source-driver.py`
under `architecture-bootstrap-source-protocol.md`. Its actual public-TC compiler
consumers, numbered documentation references and real Cargo graph observations
are mandatory alongside CHK-011, not replacements for this Rust/runtime corpus.
Freeze and review both new assets with this package. The extension states exact
coverage and mandatory independent production source review explicitly;
neither a broad source-obligation row nor a shared check ID proves an unlisted
negative control has run.
