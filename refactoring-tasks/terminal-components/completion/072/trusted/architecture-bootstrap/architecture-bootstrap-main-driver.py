...
def compiler_identity():
    cargo = Path(shutil.which("cargo")).absolute()
    rustup = shutil.which("rustup")
    rustc = Path(subprocess.check_output([rustup, "which", "rustc"], text=True).strip()).resolve() if rustup else Path(shutil.which("rustc")).resolve()
    return {"cargo": {"path": str(cargo), "sha256": sha(cargo.read_bytes()),
                      "version": subprocess.check_output([str(cargo), "--version"], text=True).strip()},
            "rustc": {"path": str(rustc), "sha256": sha(rustc.read_bytes()),
                      "version": subprocess.check_output([str(rustc), "--version", "--verbose"], text=True).strip()}}
...
def compile_execute(root, target, package, test, selected, kind="test"):
    ...
    env = dict(os.environ, CARGO_TARGET_DIR=str(target), RUSTFLAGS="--cap-lints=allow", CARGO_NET_OFFLINE="true")
    ...
    compilation = {"argv": argv, "exit": build.returncode, "compiler_messages_sha256": sha(build.stdout),
                   ...
                   "toolchain": compiler_identity(), "build_policy": {"locked": True, "offline": True, "rustflags": "--cap-lints=allow"}}
