...
schema: task/v5
id: TASK-035
title: "Restore all live Showcase Settings flows"
kind: refactor
---

# TASK-035 — Restore all live Showcase Settings flows
...
General, Members and Environment provide original editing/save/dirty/selection/prompt flows. The application report shows that a migrated file, a green self-baseline or a generic component call can coexist with wrong live behavior. This task proves actual public-component state transitions and oracle output, not nominal API use.
...
- **P-001:** Accepted receipts for `terminal-components/completion/034`, `terminal-components/completion/024` exist, their accepted source commits are ancestors of the integrated parent, and the whole sealed baseline/test-disposition products are present.
...
- Restore General project/description/visibility/toggles and dirty save/cancel/confirm; Members editable names/roles, validation, keyed sorted selection, removal confirmation, empty removal and invitation status; Environment range/all selection, removal, uppercase-variable prompt, append and empty state. Saved state and focus must survive tab switches and resize.
- Exact owned IDs: `APP:SC-BASE-settings`, `APP:SC-SETTINGS-GENERAL`, `APP:SC-SETTINGS-MEMBERS`, `APP:SC-SETTINGS-ENV`.
...
- **R-001 (MUST):** Restore General project/description/visibility/toggles and dirty save/cancel/confirm; Members editable names/roles, validation, keyed sorted selection, removal confirmation, empty removal and invitation status; Environment range/all selection, removal, uppercase-variable prompt, append and empty state. Saved state and focus must survive tab switches and resize. Every exact row and checkpoint obligation in `trusted/obligations.md` must compare equal against the sealed oracle in its declared direct and PTY lanes; no text-only, initial-frame-only or retargeted-coordinate substitute is acceptable.
- **R-002 (MUST):** Live Field/Form, Grid, List, Tabs and Dialog composition must own displayed values and transitions. Remove paint_historical and local General-field appearance facsimiles; no hidden enabled control or read-only substitute for an editable oracle field.
...
- **D-001:** Live Field/Form, Grid, List, Tabs and Dialog composition must own displayed values and transitions. Remove paint_historical and local General-field appearance facsimiles; no hidden enabled control or read-only substitute for an editable oracle field.
