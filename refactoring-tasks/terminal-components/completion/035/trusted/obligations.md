# TASK-035 trusted obligations

## Re-audit shared API consumer contract

ADJ-10: source-qualified Settings radio navigation commits through the shared typed event or Form::radio_navigation_commits(true); preserve exact key/value/cursor/callback observations and choice geometry. This is binding under R-001/AC-001/CHK-004, R-002/AC-002/CHK-006 and R-003/AC-003/CHK-005. Direct and full application traces must prove the actual source composition; an API declaration or compile-only adaptation does not close parity.

This file is part of the immutable task package, not candidate-writable configuration. It binds R-001/AC-001/CHK-004 and R-002/AC-002/CHK-006. Source rows are copied without changing their action or assertion content from the showcase scenario register. Every copied field below is normative except its historical capture-status description, which is not a parity verdict.

## Observable outcome

Restore General project/description/visibility/toggles and dirty save/cancel/confirm; Members editable names/roles, validation, keyed sorted selection, removal confirmation, empty removal and invitation status; Environment range/all selection, removal, uppercase-variable prompt, append and empty state. Saved state and focus must survive tab switches and resize.

## Architecture and non-regression

Live Field/Form, Grid, List, Tabs and Dialog composition must own displayed values and transitions. Remove paint_historical and local General-field appearance facsimiles; no hidden enabled control or read-only substitute for an editable oracle field.

All task-owned, prerequisite and previously closed scenarios must pass. Execute the complete required inventory without fail-fast and account for every actual result. Only exact unfinished future-owner failures in the immutable stage map may remain; missing execution, new failures, unexpected errors, changed classification or reopening a closed scenario fail. Whole-workspace build, MSRV compile, formatting, lint and compatible architecture checks must pass. A diagnostic failure is never relabelled a parity pass.

## Trust and expansion

The oracle commit is 02f5294bfdbf38004cc49130d0aff1d01f31434c; main architecture starts at 7b27732a8c3c131760ec3438f641cb3c11343a42. Accepted prerequisite receipts, taskfmt afd3b575dbcc7044620bec4b9493a74eca3e5ef2 and the independently qualified proof executable are host-owned. Materialized actions, numeric oracle coordinates, scenario/checkpoint membership, palettes, clocks, fonts, profiles, semantic mappings and exact expected cells are sealed before this task starts. Candidate code cannot read expected artifacts or write the host catalog, receipts, refs, comparator or another run. The dispatcher/bootstrap is produced by TASK-001, runners by TASK-070, accounting by TASK-071 and architecture probes by TASK-072. These commands are prerequisite deliverables, not evidence that they already execute.

The host supplies the immutable full application expansion grammar and proof contract alongside this file. Expand all recorded sizes, colors, focus stops, targets, ticks, source-qualified test bodies and checkpoint boundaries exactly as the accepted baseline did. Preserve every original assertion in ROUTE seeds. Source-only helpers and SCAN/SWEEP membership are materialized before this task; the executor never expands against its own implementation. Capture full row-major schema-3 cells, blank/wide continuation cells, modifiers, cursor and semantic transitions after every listed checkpoint. Preview/state-only or exact-clock lanes remain honest; executable reachability and terminal lifecycle require an actual process and owned PTY. Both capture operations must produce an explicit complete result, including accepted non-applicability rather than omitted output.

Cross-cutting full-app rows may be closure-owned; earlier tasks still preserve the subset already closed in the host ledger. Final closure replays the union on one exact tree and requires zero application-unresolved identities. Future-owned diagnostic failures are reported as failures, never success.

## Exact scenario contracts

### APP:SC-BASE-settings

- **page:** settings
- **sizes:** 80x24,100x30,120x40,160x50
- **action_checkpoints:** fresh draw
- **required_observable_proof:** General tab; defaults and save state
- **source_refs:** O:pages/settings.rs:158
- **components:** Tabs,Field,Table,List,Dialog,Button

### APP:SC-SETTINGS-GENERAL

- **page:** settings
- **sizes:** 80x24,120x40
- **action_checkpoints:** focus(Project name);Enter;End;type(-new);Enter;focus(Description);Enter;paste(New\nnotes);Esc;focus(Visibility);Down;Space;click Auto-merge approved PRs;click Protect main branch;click Save changes;Esc;click Save changes;Enter;Ctrl+S
- **required_observable_proof:** Live edits; dirty marker; confirm cancel preserves dirty; confirm/direct save clears dirty; exact footer
- **source_refs:** O:pages/settings.rs:276,494
- **components:** Tabs,Field,TextArea,RadioGroup,Toggle,Dialog

### APP:SC-SETTINGS-MEMBERS

- **page:** settings
- **sizes:** 80x24,120x40
- **action_checkpoints:** focus(tabs);2;focus(Members table);Down;Right*2;Enter;Ctrl+U;type(invalid);Enter;Esc;Enter;Ctrl+U;type(Admin);Enter;move Name column;Enter;Ctrl+U;Enter;assert name validation error;type(Renamed member);Enter;assert Member updated;click Name column header;assert selected member identity;click Remove…;Esc;reopen;y;click Invite;repeat confirmed remove until empty
- **required_observable_proof:** Role and nonempty-name validation; valid name/role edits; sorting preserves selected identity; cancel/confirm text includes selected name+role; confirmed removal deletes that identity; empty/disabled removal; invitation status
- **source_refs:** O:pages/settings.rs:28,276,494,555
- **components:** Tabs,Table,Dialog,Button

### APP:SC-SETTINGS-ENV

- **page:** settings
- **sizes:** 80x24,120x40
- **action_checkpoints:** focus(tabs);3;focus(Environment list);Space;Shift+Down;click Remove selected;click Add variable…;Enter;type(lower-case);Enter;assert variable-name rejection and editing false;Enter;Ctrl+U;type(API_BASE_URL);Enter;assert Added API_BASE_URL and closed dialog;focus(Environment list);a;click Remove selected
- **required_observable_proof:** Multi-selection/removal; variable uppercase validation and explicit edit re-entry; appended empty value; all-remove empty state and disabled action
- **source_refs:** O:pages/settings.rs:38,276,494,569
- **components:** Tabs,List,Dialog,TextInput

## Correction boundary

If exact parity requires a shared-component fix, unsupported public extension, changed source authority or new trusted fixture, stop NEEDS_REPLAN and name the responsible owner. Do not duplicate mechanics inside this app or edit expected evidence. A completed task produces host-verified results and an accepted tree receipt, not a candidate-written approval.
