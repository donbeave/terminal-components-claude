...
schema: task/v5
id: TASK-012
title: "Preserve pure layout clipping and split geometry"
kind: bugfix
---

# TASK-012 — Preserve pure layout clipping and split geometry

## Goal

Pure measurement and SplitPane allocation share exact clipping and pointer geometry.
...
- `crates/tui/src/layout.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/measure.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/split.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_012.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** Exercise fixed/flex/Auto and measured Auto allocation, insets, wrapping, truncation, zero/one-cell extents and nonzero origins. SplitPane alone returns two logical body rectangles excluding seam; horizontal/vertical minima, drag grab offset, collapse, maximize and resize retain oracle ratios and targets. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Measure performs no state mutation, registration or allocation and follows the same style accumulation as paint. Container callbacks run once even under empty clipping; empty leaves paint/register nothing. No public bypass exposes duplicated split geometry. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Negative cases deliberately attempt paint/registration from an empty body and stale resize hits. Compare first-frame layout facts and scroll-position labels, half-open boundaries, offset seam hitboxes and 1000 repeated measures with unchanged durable state and allocation counters. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed layout-measure and split-pane oracle cases and the exact task history clauses
...
```gherkin
Given the live layout-measure and split-pane implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane layout-measure and split-pane actions and checkpoints
...
- **D-001:** Measure performs no state mutation, registration or allocation and follows the same style accumulation as paint. Container callbacks run once even under empty clipping; empty leaves paint/register nothing. No public bypass exposes duplicated split geometry.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_012 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
