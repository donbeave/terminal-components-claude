...
schema: task/v5
id: TASK-044
title: "Restore Holla activity stdin cancellation and plan output"
kind: refactor
---

# TASK-044 — Restore Holla activity stdin cancellation and plan output
...
Output streams/find/selection/stdin/EOF/cancellation and executing-plan output/tab lifetime match oracle World ownership. The application report shows that a migrated file, a green self-baseline or a generic component call can coexist with wrong live behavior. This task proves actual public-component state transitions and oracle output, not nominal API use.
...
- **P-001:** Accepted receipts for `terminal-components/completion/043`, `terminal-components/completion/025` exist, their accepted source commits are ancestors of the integrated parent, and the whole sealed baseline/test-disposition products are present.
...
- Restore running/succeeded/failed/stopping/stopped/launch-failure activity output, exact mixed streams/filter chips/retention loss/find/selection/follow; available/waiting/plain/password stdin, Enter/EOF/cancel and TERM→KILL settlement; plan step/output focus/maximize/retry/failure/dependent blocked/cancel; activity picker, running close and quit settlement with tab restoration. Secret input must not remain in frames, logs or retained stdin bytes.
- Exact complete primary IDs: `APP:HO-BASE-08`, `APP:HO-BASE-10`, `APP:HO-BASE-25`, `APP:HO-BASE-26`, `APP:HO-HP02`, `APP:HO-HP03`, `APP:HO-HP15`, `APP:HO-ROUTE-04`, `APP:HO-ROUTE-05`, `APP:HO-ROUTE-15`, `APP:HO-ROUTE-26`, `APP:HO-ROUTE-32`, `APP:HO-ROUTE-38`, `APP:HO-ROUTE-41`, `APP:HO-ROUTE-42`, `APP:HO-QUIT`, `APP:HO-CLOSE`, `APP:HO-PICKER`, `APP:HO-OUTPUT-FIND`, `APP:HO-STDIN`.
...
- **R-001 (MUST):** Restore running/succeeded/failed/stopping/stopped/launch-failure activity output, exact mixed streams/filter chips/retention loss/find/selection/follow; available/waiting/plain/password stdin, Enter/EOF/cancel and TERM→KILL settlement; plan step/output focus/maximize/retry/failure/dependent blocked/cancel; activity picker, running close and quit settlement with tab restoration. Secret input must not remain in frames, logs or retained stdin bytes. Complete parent scenarios are exactly those in `trusted/scenario-ids.tsv`. Required early contributions are exactly the nonempty protected rows in `trusted/stage-contributions.tsv`; frame contributions compare complete selected checkpoints under AC-001/CHK-004, while semantic contributions are independently checked under AC-008/CHK-006. A contribution never closes its parent. Full parent transcripts execute diagnostically until their recorded complete owner, and every prior closed contribution remains required. `trusted/holla-route-expansion.md` requires unchanged native assertions at native source dimensions plus separately mapped oracle-derived size captures; no assertion deletion, candidate coordinate selection, masks or fabricated whole-parent pass. B-HO-09 Activity branches in trusted/holla-branch-corrections.md are required under HO-OUTPUT-FIND: output-find-paste-trim, output-find-escape-keeps-follow-paused, output-find-live-coordinate-reconcile and output-find-empty-no-match. Reveal pauses follow; Escape clears find and explicit End resumes follow. Recompute keeps an exact surviving visible-line/byte-range tuple or chooses zero, not a universal new-query reset.
- **R-002 (MUST):** World owns running work across navigation; TextViewport/editor core, ChipBar, Picker/Dialog and runtime own interaction mechanics. Preserve incremental append rather than rebuilding retained output; cancellation is domain progress, not disappearance of a page. Use virtual Clock and finite executor outcomes only. Preserve append/scalar-pop query grammar, stdin-before-find paste and incremental output through shared configured editing/matching/TextViewport operations. Do not replace tuple-coordinate retention with stable-content selection or add Snapshot/plan/trust/Jackin Inspect search.
...
- **D-009:** HO-044-F05 retains every native ActivitiesMulti route assertion and complete local Activity/picker/monitor frames. The full HO-ACTIVITIES SWEEP can activate its seeded Review four modified files button and therefore closes047; emitting that intent is not a full Git snapshot pass.
