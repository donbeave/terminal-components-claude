...
- **P-004:** TASK-073's independently qualified production style-measurement seam is present in the integrated ancestry and preserved by TASK-011. The verifier has recorded its exact boundary census, three-mode measurement protocol, compiler/profile, warmup/batch schedule and measurement-error rule before this candidate. No candidate-chosen timer calibration, denominator or sampling selector is accepted.
...
- **D-001:** This is proof completion, not authority to repair arbitrary production performance defects in test scope. A failed bound returns to its component/application owner. Fixed before rows retain their original SHA and units; candidate performance results cannot overwrite them.
