...
schema: task/v5
id: TASK-047
title: "Restore Holla repository and build-task outcome worlds"
kind: refactor
---

# TASK-047 — Restore Holla repository and build-task outcome worlds
...
- **P-001:** Accepted receipts for `terminal-components/completion/046` exist, their accepted source commits are ancestors of the integrated parent, and the whole sealed baseline/test-disposition products are present.
...
- Restore current-root Git status/fetch/prune/gc and sibling push/status batches with exact membership/per-repository failures; native task-source names/caps/odd-name diagnostics; Cargo exit/output/clean target effects; Gradle wrapper/task/daemon/bounded recursive cleanup; IntelliJ metadata case handling and failed operation-log writes. Every simulated command retains exact argv/cwd/host and final world outcome.
- Exact complete primary IDs: `APP:HO-BASE-16`, `APP:HO-BASE-17`, `APP:HO-BASE-18`, `APP:HO-BASE-19`, `APP:HO-BASE-22`, `APP:HO-BASE-23`, `APP:HO-HP05`, `APP:HO-HP06`, `APP:HO-HP07`, `APP:HO-HP08`, `APP:HO-HP11`, `APP:HO-HP12`, `APP:HO-HP14`, `APP:HO-ROUTE-22`, `APP:HO-ROUTE-23`, `APP:HO-ACTIVITIES`, `APP:HO-OUTPUT-SCROLL`.
...
- **R-001 (MUST):** Restore current-root Git status/fetch/prune/gc and sibling push/status batches with exact membership/per-repository failures; native task-source names/caps/odd-name diagnostics; Cargo exit/output/clean target effects; Gradle wrapper/task/daemon/bounded recursive cleanup; IntelliJ metadata case handling and failed operation-log writes. Every simulated command retains exact argv/cwd/host and final world outcome. Complete parent scenarios are exactly those in `trusted/scenario-ids.tsv`. Required early contributions are exactly the nonempty protected rows in `trusted/stage-contributions.tsv`; frame contributions compare complete selected checkpoints under AC-001/CHK-004, while semantic contributions belonging to earlier tasks retain their own architecture-check identities. A contribution never closes its parent. Full parent transcripts execute diagnostically until their recorded complete owner, and every prior closed contribution remains required. `trusted/holla-route-expansion.md` requires unchanged native assertions at native source dimensions plus separately mapped oracle-derived size captures; no assertion deletion, candidate coordinate selection, masks or fabricated whole-parent pass.
- **R-002 (MUST):** Domain adapters choose finite fixture outcomes; shared snapshot/activity/review composition remains generic and live. No real Git/Cargo/Gradle/IDEA processes or provider integration, no 'Would run' placeholder replacing required modeled outcome, and no duplicated command table widgets.
...
- **D-007:** This task additionally closes the whole HP14 and Output-scroll parents, including their second ParityGitBatch constructor, Push four repositories membership/mode/start assertions and all earlier retained-output contributions. The initial executor branch alone cannot close either parent.
...
- **D-001:** Domain adapters choose finite fixture outcomes; shared snapshot/activity/review composition remains generic and live. No real Git/Cargo/Gradle/IDEA processes or provider integration, no 'Would run' placeholder replacing required modeled outcome, and no duplicated command table widgets.
...
- **D-009:** Close the full HO-ACTIVITIES SWEEP, including the actual seeded Review four modified files follow-up dispatch to git.review and its complete Snapshot frame. Preserve all native route assertions and HO-044-F05 contributions.
