...
Restore current-root Git status/fetch/prune/gc and sibling push/status batches with exact membership/per-repository failures; native task-source names/caps/odd-name diagnostics; Cargo exit/output/clean target effects; Gradle wrapper$TASK_DIR/daemon/bounded recursive cleanup; IntelliJ metadata case handling and failed operation-log writes. Every simulated command retains exact argv/cwd/host and final world outcome.
...
- **D-007:** This task additionally closes the whole HP14 and Output-scroll parents, including their second ParityGitBatch constructor, Push four repositories membership/mode/start assertions and all earlier retained-output contributions. The initial coordinator branch alone cannot close either parent.
