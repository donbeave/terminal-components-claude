...
schema: task/v5
id: TASK-045
title: "Restore Holla disk scan tree and insight navigation"
kind: refactor
---

# TASK-045 — Restore Holla disk scan tree and insight navigation
...
- **P-001:** Accepted receipts for `terminal-components/completion/044` exist, their accepted source commits are ancestors of the integrated parent, and the whole sealed baseline/test-disposition products are present.
...
- Restore streaming disk scans, allocated/apparent bytes, partial errors/cancel/cache hints, home/custom paths; hierarchy/folding/parent dominance/lazy insertion stable selections, top-files/Spotlight and facts; complete insight category sizes/eligibility/process guards; macOS/Linux capability differences and unavailable features. Idle finder and disk ticks rebuild nothing; protected paths remain ineligible.
- Exact complete primary IDs: `APP:HO-BASE-28`, `APP:HO-BASE-29`, `APP:HO-BASE-30`, `APP:HO-BASE-33`, `APP:HO-BASE-34`, `APP:HO-HP18`, `APP:HO-HP20`, `APP:HO-ROUTE-13`, `APP:HO-ROUTE-20`, `APP:HO-ROUTE-28`, `APP:HO-ROUTE-29`, `APP:HO-ROUTE-33`, `APP:HO-ROUTE-39`, `APP:HO-FILES-ACTIONS`, `APP:HO-DISK-SCAN`.
...
- **R-001 (MUST):** Restore streaming disk scans, allocated/apparent bytes, partial errors/cancel/cache hints, home/custom paths; hierarchy/folding/parent dominance/lazy insertion stable selections, top-files/Spotlight and facts; complete insight category sizes/eligibility/process guards; macOS/Linux capability differences and unavailable features. Idle finder and disk ticks rebuild nothing; protected paths remain ineligible. Complete parent scenarios are exactly those in `trusted/scenario-ids.tsv`. Required early contributions are exactly the nonempty protected rows in `trusted/stage-contributions.tsv`; frame contributions compare complete selected checkpoints under AC-001/CHK-004, while semantic contributions are independently checked under AC-008/CHK-006. A contribution never closes its parent. Full parent transcripts execute diagnostically until their recorded complete owner, and every prior closed contribution remains required. `trusted/holla-route-expansion.md` requires unchanged native assertions at native source dimensions plus separately mapped oracle-derived size captures; no assertion deletion, candidate coordinate selection, masks or fabricated whole-parent pass.
- **R-002 (MUST):** Tree/Grid/Props/ScrollRegion/Progress/Meter must paint the same stable domain rows used by events. Preserve virtual scan ownership and capability facts; no real disk traversal, process probes, deletion or faked platform availability.
...
- **D-007:** This task closes full Files Reveal-to-Activity-to-Analyze-Disk and Go-menu-to-Disk parents, cleanup-category fade, and the HO-045-F/S Disk contributions. Trash review, cleanup effects and Linux no-fallback report close046. Static platform capabilities do not authorize any real host operations.
...
- **D-001:** Tree/Grid/Props/ScrollRegion/Progress/Meter must paint the same stable domain rows used by events. Preserve virtual scan ownership and capability facts; no real disk traversal, process probes, deletion or faked platform availability.
