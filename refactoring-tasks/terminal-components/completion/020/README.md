...
schema: task/v5
id: TASK-020
title: "Restore keyed collection selection navigation and fades"
kind: bugfix
---

# TASK-020 — Restore keyed collection selection navigation and fades

## Goal

Keyed collections preserve exact selection, branch and strip journeys.
...
- `trusted/obligations.md` in full, including every assigned historical clause.
- `trusted/source-witnesses.md` in full; its finite case IDs, source anchors, inputs, axes and exact checkpoints are mandatory host-fixed proof inputs, not coordinator-selected examples.
- `trusted/source-obligations.tsv` in full; its exact source-qualified clauses, dispositions and task/check mappings are normative, not optional background.
- `../../../../docs/refactoring-plan/components.md`, `architecture.md`, `architecture-adjudication.md` and `proof-contract.md`.
- Every pinned oracle/main source and existing test named in the trusted family/historical rows.

## Preconditions

- **P-001:** The protected campaign record has accepted TASK-001/070/071/072 products, sealed complete baseline TASK-006 and exact inventory/dispositions TASK-007/008; every listed predecessor is accepted and integrated into the recorded parent.
- **P-002:** Source pins, taskfmt `afd3b575dbcc7044620bec4b9493a74eca3e5ef2`, reviewed tui-snap revision, toolchain/lock and immutable catalog/context identities resolve from protected receipts.
- **P-003:** The verifier recorded exact owned scenario membership, source-qualified non-applicability, numeric traces and new-test obligations; the coordinator cannot choose expected output or expand write scope.
...
- `crates/tui/src/components/mod.rs` and `crates/tui/src/lib.rs` only for the fixed ADJ-16 Steps presentation re-exports.
- `crates/tui/src/theme/recipe.rs` and `crates/tui/src/theme/builtin/mod.rs` only for append-only RAIL identity and the isolated Steps Rail recipe; preserve DEFAULT and all other families.

- `apps/showcase/src/pages/chips.rs` only for exhaustive matching of the new keyless ChipBarAction variants so the workspace compiles; preserve existing item-action behavior. TASK-037 owns the actual source-compatible application migration and domain handling.

- `crates/tui/tests/conformance.rs` only for exhaustive ChipBarAction compatibility with keyless LeadRequested/ClearRequested; preserve every original test identity.

- `crates/tui/src/components/list.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/nav_list.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/tree.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/steps.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/chip.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/tabs.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_020.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** Restore List current-row completed click/range selection, NavList Chose versus EnterContent and compact sections, Tree lazy branch/filter/replacement semantics, Steps skipped inspectability/frontier, Chip checked/close/add/overflow and Tabs active/hover/cursor/close/reorder geometry. Apply shared fades with current row protection. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Keep borrowed models and stable identities; Tree cache stores derived structure only and filtering projects matches plus ancestors without changing saved expansion. AddRequested has no fake item key; domain shell alone focuses content after EnterContent. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Run duplicate labels, external insert/delete/reorder, removed current/selected target, lazy update during input, filter clear restoration, disabled branch, full/narrow strip and first/last boundary. Prove 100k visible-only callbacks and incremental subtree/frontier work with saturation/revision invalidation. Reject pointer hover impersonating activation or stale index retargeting. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
- **R-005 (MUST):** The full subagent-verified canonical gate passes on the fixed exact tree, with protected-input integrity, actual prerequisite ancestry and complete check evidence.

## Acceptance criteria

### AC-001 — Owned behavior matches exact oracle
```gherkin
Given the sealed list and nav-list and tree and steps and chips and tabs oracle cases and the exact task history clauses
...
```gherkin
Given the live list and nav-list and tree and steps and chips and tabs implementation and independent ownership witnesses
...
```gherkin
Given the fixed required test identities and the task-specific negative cases in R-003
...
```gherkin
Given the fixed direct-lane list and nav-list and tree and steps and chips and tabs actions and checkpoints
When the real production update and draw paths run on the fixed candidate
...
```gherkin
Given the fixed full-application PTY diagnostic corpus and its immutable correction-owner stage map
...
- **D-008:** Apply ADJ-16 and its exact typed-policy/default/ownership contract in `trusted/obligations.md`. Finite `trusted/source-witnesses.md` rows bind R-001/R-002/R-003 through CHK-002/004/005/006; no deferred API design or app-local substitute remains.

- **D-007:** Apply ADJ-11 and the fixed re-audit ChipBar contract in `trusted/obligations.md`, including exact borrowed public builders, keyless events, policy defaults, source-compatible app builder composition and conformance compatibility. CHK-002/004 capture exact cells/actions, CHK-005 executes negatives, and CHK-006 proves one shared owner.

- **D-001:** Keep borrowed models and stable identities; Tree cache stores derived structure only and filtering projects matches plus ancestors without changing saved expansion. AddRequested has no fake item key; domain shell alone focuses content after EnterContent.
...
- **D-005:** Future `$WORKTREE/tools/refactor-proof/bin/tc-proof` operations are installed only from accepted producer receipts: TASK-001 host/comparator core, TASK-070 preflight/required/oracle/capture/close, TASK-071 account-tests and TASK-072 architecture. No candidate-local substitute or recursive taskfmt gate is permitted.
- **D-006:** Oracle comparison CHK-004 covers this task's fixed component-owned direct fixtures and already closed required cases. Full production application PTY frames remain owned by their app correction/closure tasks; CHK-003 captures their complete diagnostic evidence and CHK-005 records every actual mismatch under the immutable stage map. Capture completeness is not parity. No app frame is cropped, masked, rewritten or treated as an empty passing set, and this task cannot depend on an unrepaired application's full-frame equality. Component-sensitive PTY lifecycle/behavior obligations use source-qualified applicability and their separately owned architecture/behavior probes; final app closure compares all complete PTY frames.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_020 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
    - [ ] **2.2** Preserve borrowed ownership, attributed paint and the task-specific architecture contract. (`R-002`, `AC-002`, `CHK-006`)
    - [ ] **2.3** Capture all fixed direct production trajectories. (`R-001`, `AC-006`, `CHK-002`)
