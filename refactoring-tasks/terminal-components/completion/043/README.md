...
schema: task/v5
id: TASK-043
title: "Restore Holla arguments trust review and two-stage gates"
kind: refactor
---

# TASK-043 — Restore Holla arguments trust review and two-stage gates
...
Arguments, trust, plan review and gate1/gate2 restore exact UI while preserving one-shot/current-target checks. The application report shows that a migrated file, a green self-baseline or a generic component call can coexist with wrong live behavior. This task proves actual public-component state transitions and oracle output, not nominal API use.
...
- **P-001:** Accepted receipts for `terminal-components/completion/042`, `terminal-components/completion/019`, `terminal-components/completion/023` exist, their accepted source commits are ancestors of the integrated parent, and the whole sealed baseline/test-disposition products are present.
...
- Restore structured arguments with pointer editing, defaults/validation/Ctrl+S/cancel; whole-file digest/path/cwd/literal argv trust and changed-byte/save-failure refusal; review dependency selection/exclusion/undo/facts; gate1 Cancel/Continue and gate2 acknowledgement/exact phrase/disabled submit/drift invalidation. Preserve no effect before current reviewed target acceptance, exact outer-click policy and modal paste ownership.
- Exact complete primary IDs: `APP:HO-ROUTE-27`, `APP:HO-ROUTE-31`, `APP:HO-ROUTE-40`, `APP:HO-PLAN-REVIEW`, `APP:HO-OVERLAY-PASTE`.
...
- **R-001 (MUST):** Restore structured arguments with pointer editing, defaults/validation/Ctrl+S/cancel; whole-file digest/path/cwd/literal argv trust and changed-byte/save-failure refusal; review dependency selection/exclusion/undo/facts; gate1 Cancel/Continue and gate2 acknowledgement/exact phrase/disabled submit/drift invalidation. Preserve no effect before current reviewed target acceptance, exact outer-click policy and modal paste ownership. Complete parent scenarios are exactly those in `trusted/scenario-ids.tsv`. Required early contributions are exactly the nonempty protected rows in `trusted/stage-contributions.tsv`; frame contributions compare complete selected checkpoints under AC-001/CHK-004, while semantic contributions are independently checked under AC-008/CHK-006. A contribution never closes its parent. Full parent transcripts execute diagnostically until their recorded complete owner, and every prior closed contribution remains required. `trusted/holla-route-expansion.md` requires unchanged native assertions at native source dimensions plus separately mapped oracle-derived size captures; no assertion deletion, candidate coordinate selection, masks or fabricated whole-parent pass.
- **R-002 (MUST):** Use Form/Field/Select/Dialog and runtime layers for shared mechanics. Preserve main's compatible one-shot/current-target/revision authorization and domain checks while restoring oracle visible gates. Never replace a live gate with a text picture or synchronously fake executing-plan progress.
...
- **D-009:** Preserve the exact Args and paste branches in `trusted/holla-trace-corrections.md`. Ctrl+L selects all; Ctrl+A means Home. Required emptiness is the only port field validation; arbitrary nonempty values retain the oracle Snapshot fallback. True modals isolate Args; F10 MenuBar preserves source paste passthrough through the shared configured editor operation. Never normalize this difference away or duplicate insertion in the app.

- **D-007:** This task closes complete Args/trust/review-only journeys and HO-043-F/S contributions. Static fixture registration for the source ParityDiskNavigation argument form is permitted; no Disk renderer is included. Port5173 Snapshot closes048, accepted custom-action outcomes049, and the complete Docker execution seed048.
...
- **D-001:** Use Form/Field/Select/Dialog and runtime layers for shared mechanics. Preserve main's compatible one-shot/current-target/revision authorization and domain checks while restoring oracle visible gates. Never replace a live gate with a text picture or synchronously fake executing-plan progress.
