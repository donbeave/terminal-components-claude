...
schema: task/v5
id: TASK-018
title: "Restore controlled choices Select and mono label geometry"
kind: bugfix
---

# TASK-018 — Restore controlled choices Select and mono label geometry

## Goal

Controlled choices preserve cursor/value distinction and mono label geometry.
...
- `crates/tui/src/components/mod.rs` and `crates/tui/src/lib.rs` only for the fixed ADJ-15 Select policy re-exports.

- `crates/tui/tests/conformance.rs` only for exhaustive RadioGroupAction compatibility and retained controlled-value assertions; preserve original test identities.
- `crates/tui/src/components/form.rs` only for compile-safe exhaustive action adaptation after Navigated is introduced, preserving the default no-navigation-commit policy; TASK-019 owns the opt-in oracle Form bridge.

- `crates/tui/src/components/choice.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/select.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_018.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** Checkbox/Toggle values remain caller-controlled; RadioGroup stable chosen key is distinct from cursor and only seeds missing navigation. Select closed/open markers preserve exact reserved position, strip false field SELECTED and show popup chosen once. Complete the real mono_pressed_choice_keeps_the_label_geometry proof. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Use shared fields/rows/runtime popup; no item-aware public FieldControl widening. Preserve source item ownership, effective inherited/configured disable and explicit glyph Set/Clear over defaults. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Compare narrow/full/zero clipping, checked/unchecked/pressed/hovered/disabled all-color cells and complete pointer containment. External value change/reorder/removal does not retarget established cursor. Select Esc/focus-out closes correctly; installed readiness slot differs from single-glyph override; no label truncation from brackets. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed checkbox-toggle and radio-group and select oracle cases and the exact task history clauses
...
```gherkin
Given the live checkbox-toggle and radio-group and select implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane checkbox-toggle and radio-group and select actions and checkpoints
...
- **D-008:** Apply ADJ-15 and its exact typed-policy/default/ownership contract in `trusted/obligations.md`. Finite `trusted/source-witnesses.md` rows bind R-001/R-002/R-003 through CHK-002/004/005/006; no deferred API design or app-local substitute remains.
...
- **D-001:** Use shared fields/rows/runtime popup; no item-aware public FieldControl widening. Preserve source item ownership, effective inherited/configured disable and explicit glyph Set/Clear over defaults.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_018 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
