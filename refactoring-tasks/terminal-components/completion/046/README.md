...
schema: task/v5
id: TASK-046
title: "Restore Holla simulated cleanup ownership and reports"
kind: refactor
---

# TASK-046 — Restore Holla simulated cleanup ownership and reports
...
- **P-001:** Accepted receipts for `terminal-components/completion/045` exist, their accepted source commits are ancestors of the integrated parent, and the whole sealed baseline/test-disposition products are present.
...
- Restore Trash/permanent/dry-run selection/review/phrase/revision and commit-time target validation; deny protected roots/user paths and drift/races; preserve selected-link-only effects, Trash collisions/retry/refusal and no permanent-delete fallback. World owns in-flight per-item cleanup, exact freed-byte/report/APFS wording/log failures/history links; quit waits until settlement and report persistence. Built-in find/browse/disk/cleanup remain available with an empty toolset.
- Exact complete primary IDs: `APP:HO-BASE-06`, `APP:HO-BASE-31`, `APP:HO-BASE-32`, `APP:HO-HP19`, `APP:HO-HP21`, `APP:HO-HP22`, `APP:HO-HP22-QUIT`, `APP:HO-HP23`, `APP:HO-ROUTE-07`, `APP:HO-ROUTE-18`, `APP:HO-ROUTE-30`, `APP:HO-DISK-TREE`, `APP:HO-CLEANUP-MODES`, `APP:HO-CLEANUP-SAFETY`, `APP:HO-CLEANUP-REPORT`.
...
- **R-001 (MUST):** Restore Trash/permanent/dry-run selection/review/phrase/revision and commit-time target validation; deny protected roots/user paths and drift/races; preserve selected-link-only effects, Trash collisions/retry/refusal and no permanent-delete fallback. World owns in-flight per-item cleanup, exact freed-byte/report/APFS wording/log failures/history links; quit waits until settlement and report persistence. Built-in find/browse/disk/cleanup remain available with an empty toolset. Complete parent scenarios are exactly those in `trusted/scenario-ids.tsv`. Required early contributions are exactly the nonempty protected rows in `trusted/stage-contributions.tsv`; frame contributions compare complete selected checkpoints under AC-001/CHK-004, while semantic contributions belonging to earlier tasks retain their own architecture-check identities. A contribution never closes its parent. Full parent transcripts execute diagnostically until their recorded complete owner, and every prior closed contribution remains required. `trusted/holla-route-expansion.md` requires unchanged native assertions at native source dimensions plus separately mapped oracle-derived size captures; no assertion deletion, candidate coordinate selection, masks or fabricated whole-parent pass.
- **R-002 (MUST):** Keep compatible current-target/one-shot safety and share Dialog/Field/Props/Tree/ScrollRegion mechanics. All filesystem effects are oracle virtual-FS state mutations; never connect an OS deletion provider or treat simulated UI acceptance as authority for a real destructive action.
...
- **D-007:** This task additionally closes whole HP19/Disk-tree journeys that enter Trash review, and HP23 platform journeys that finish with Linux no-fallback cleanup/report/opener refusal. It requires all earlier Disk contributions to remain passing. The mixed cleanup/PostgreSQL whole parent closes048.
...
- **D-001:** Keep compatible current-target/one-shot safety and share Dialog/Field/Props/Tree/ScrollRegion mechanics. All filesystem effects are oracle virtual-FS state mutations; never connect an OS deletion provider or treat simulated UI acceptance as authority for a real destructive action.
