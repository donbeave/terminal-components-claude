...
```gherkin
Given the fixed direct-lane dialog and menu-context-menubar actions and checkpoints
...
- **D-BRANCH-SOURCE:** Menu reverse navigation uses one shared bounded cyclic-index operation for context menus, open MenuBar switching and closed MenuBar previous navigation. Previous from index zero in an enabled three-item collection reaches index two; unsigned wrapping modulo is not equivalent to signed Euclidean cycling. W-023-07 records three/five-item boundaries and disabled scanning without changing accepted forward navigation, typed action remapping or runtime layer ownership. Empty/all-disabled paths terminate without selecting or activating an unavailable item; each mode's existing source-qualified close/open response remains distinct.
