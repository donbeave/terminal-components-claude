...
schema: task/v5
id: TASK-049
title: "Restore Holla config upgrade and snapshot outcomes"
kind: refactor
---

# TASK-049 — Restore Holla config upgrade and snapshot outcomes
...
Custom config/trust diagnostics, upgrade manager plans and every remaining emitted snapshot/report parameter route preserve exact oracle finite outcomes. The application report shows that a migrated file, a green self-baseline or a generic component call can coexist with wrong live behavior. This task proves actual public-component state transitions and oracle output, not nominal API use.
...
- **P-001:** Accepted receipts for `terminal-components/completion/048` exist, their accepted source commits are ancestors of the integrated parent, and the whole sealed baseline/test-disposition products are present.
...
- Restore custom action/config diagnostics, exact persisted whole-file trust drift/save failures, revoke/reload; standalone upgrade manager commands and exact multi-stage exclusion/dependency/failure/retry results. Close every emitted Snapshot kind/parameter/action route and actual follow-up across all34 worlds, including git/diff/stash/batch, system/processes/port, docker/compose, pg/blocking, mise/outdated, history/services/ssh/brew/reports/config; restore every File/Go/Help action through real routes.
- Exact complete primary IDs: `APP:HO-BASE-07`, `APP:HO-BASE-24`, `APP:HO-BASE-27`, `APP:HO-HP01`, `APP:HO-HP13`, `APP:HO-HP17`, `APP:HO-ROUTE-03`, `APP:HO-ROUTE-25`, `APP:HO-MENUS`, `APP:HO-FINDER-SWEEP`, `APP:HO-TRUST`, `APP:HO-UPGRADE`, `APP:HO-PLAN-EXEC`, `APP:HO-SNAPSHOTS`.
...
- **R-001 (MUST):** Restore custom action/config diagnostics, exact persisted whole-file trust drift/save failures, revoke/reload; standalone upgrade manager commands and exact multi-stage exclusion/dependency/failure/retry results. Close every emitted Snapshot kind/parameter/action route and actual follow-up across all34 worlds, including git/diff/stash/batch, system/processes/port, docker/compose, pg/blocking, mise/outdated, history/services/ssh/brew/reports/config; restore every File/Go/Help action through real routes. Complete parent scenarios are exactly those in `trusted/scenario-ids.tsv`. Required early contributions are exactly the nonempty protected rows in `trusted/stage-contributions.tsv`; frame contributions compare complete selected checkpoints under AC-001/CHK-004, while semantic contributions belonging to earlier tasks retain their own architecture-check identities. A contribution never closes its parent. Full parent transcripts execute diagnostically until their recorded complete owner, and every prior closed contribution remains required. `trusted/holla-route-expansion.md` requires unchanged native assertions at native source dimensions plus separately mapped oracle-derived size captures; no assertion deletion, candidate coordinate selection, masks or fabricated whole-parent pass.
- **R-002 (MUST):** Preserve generic live snapshot/plan/field/menu composition and simulated domain outcome ownership. Enumerate emitted kind/parameter pairs from the sealed oracle catalog, not candidate-selected cases; never direct-construct an unreachable page to claim a route. HP16 and real providers stay deferred.
...
- **D-001:** Preserve generic live snapshot/plan/field/menu composition and simulated domain outcome ownership. Enumerate emitted kind/parameter pairs from the sealed oracle catalog, not candidate-selected cases; never direct-construct an unreachable page to claim a route. HP16 and real providers stay deferred.
