...
- **P-004:** TASK-008's reviewer-approved per-call-site constructor/motion/result setup adaptation records cover every extra test-source path in the shell construction contract; original eleven fixture tuples and compatible assertions remain fixed. Missing records stop execution, not an coordinator-selected migration.
...
When the architecture operation exercises every required shell contribution on the fixed candidate
