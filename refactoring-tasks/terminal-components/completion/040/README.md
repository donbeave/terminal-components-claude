...
schema: task/v5
id: TASK-040
title: "Restore Holla page stack tabs keys and CLI semantics"
kind: refactor
---

# TASK-040 — Restore Holla page stack tabs keys and CLI semantics
...
Permanent Here/page stack, persistent activity/plan tabs, CLI frame semantics, exact keys, menus and restoration work on main's App/Cx/Ui. The application report shows that a migrated file, a green self-baseline or a generic component call can coexist with wrong live behavior. This task proves actual public-component state transitions and oracle output, not nominal API use.
...
- **P-001:** Accepted receipts for `terminal-components/completion/020`, `terminal-components/completion/027`, `terminal-components/completion/030`, `terminal-components/completion/031` exist, their accepted source commits are ancestors of the integrated parent, and the whole sealed baseline/test-disposition products are present.
...
- **P-004:** TASK-008's host-approved per-call-site constructor/motion/result setup adaptation records cover every extra test-source path in the shell construction contract; original eleven fixture tuples and compatible assertions remain fixed. Missing records stop execution, not an executor-selected migration.
...
- Restore permanent Here/page-stack identity and persistent activity/plan tabs, breadcrumb/menu/chrome ownership, Alt+0–9 and missing-tab feedback, F10/F1/Ctrl+Q/Ctrl+C/Ctrl+G/Ctrl+W semantics, Escape ladder, focus restoration and exact preview CLI stdout/stderr/exit behavior. Preserve --frame units and color/motion precedence; no list/run/doctor expansion.
- Required parent transcripts (not claims of whole-scenario ownership): `APP:HO-SHELL`, `APP:HO-MENUS` (only the four HO-SHELL-C04 branches at this stage), `APP:HO-ROUTE-13`, `APP:HO-BASE-01`, `APP:HO-CLI-PREVIEW` (whole process contract).
...
- Clock/scenario/world/fixture factory seams and exact extra setup-only caller spans are bounded by trusted/holla-shell-construction-contract.md. The extra test-source paths do not authorize domain production changes or expectation edits.
...
- **R-001 (MUST):** Restore permanent Here/page-stack identity and persistent activity/plan tabs, breadcrumb/menu/chrome ownership, Alt+0–9 and missing-tab feedback, F10/F1/Ctrl+Q/Ctrl+C/Ctrl+G/Ctrl+W semantics, Escape ladder, focus restoration and exact preview CLI stdout/stderr/exit behavior. Preserve --frame units and color/motion precedence; no list/run/doctor expansion. This task closes only the exact shell contributions in `trusted/contributions.tsv` and the complete HO-CLI-PREVIEW process contract. The parent scenario transcripts still generate complete, unmasked frame/cursor/state artifacts and every frame mismatch remains an explicitly reported future-owned failure under the sealed stage map. A passing contribution never closes its parent scenario or claims any cell region equals the oracle. Complete parent-scenario equality is owned by TASK-050 after the page repair slices. The independently qualified architecture operation (CHK-006) proves the exact source-qualified shell assertions, and architecture requires every exact contribution observation/assertion to pass. No executor-selected fields, checkpoints, masks, selectors or assertion subsets are allowed. B-HO-08 in trusted/holla-branch-corrections.md requires menu-ctrl-c-captured and menu-ctrl-q-captured to keep MenuBar/process alive; after dismissal Ctrl+C exits and Ctrl+Q opens Quit. Close only HO-SHELL-C04 semantic/process assertions; HO-MENUS full parent stays049 and final closure050. B-HO-01/04 additionally require trusted/holla-shell-construction-contract.md: pure34-name parsing and source epoch/frame tick semantics; real current constructor paths; HO-SHELL-C05 and expanded C03. Do not claim all34 application launches before their existing HO-BASE owners.
- **R-002 (MUST):** Retain main's App/Cx/Ui and runtime ownership rather than resurrecting old Screen or root widget modules. Routing order is too-small, modal, menu, attached/input owner, global chord, focused shell control, page, traversal/Escape; paste has separate source dispatch: true modals capture it, while MenuBar preserves passthrough to the underlying page through shared configured field editing. Tabs and domain work have stable distinct identities. Preserve MenuBar key handling before global interrupt/quit, separate paste routing, earlier true-modal capture and retained lifecycle polling. Never suppress global Ctrl+C everywhere to satisfy menu capture. Use one canonical34 Scenario registry and typed ScenarioUnavailable construction Result, passing Motion into real fixture construction. Handle unavailable stages before terminal acquisition; preserve all eleven current real constructors. Runtime cadence and80ms simulation advancement differ; use one real App advancement seam, no fake Cx. CHK-006 enforces setup-only extra-file spans and original test tuples; final050 requires zero unavailable constructors.
...
- **D-008:** trusted/holla-shell-construction-contract.md fixes the canonical registry/typed Result API, motion-before-fixture setup, source epoch and App start-before-N-ticks order, runtime cadence versus80ms simulation increments, exact scope and staged proofs. Extra test-source edits are only008-approved mechanical constructor/motion/result setup; no assertion rewrite, filter_map, if-let skip, late domain seed or duplicate name authority. Current dependencies remain unchanged; missing future worlds are explicit future-owned failures, never accepted placeholders.

- **D-007:** B-HO-08 in trusted/holla-branch-corrections.md requires menu-ctrl-c-captured and menu-ctrl-q-captured to keep MenuBar/process alive; after dismissal Ctrl+C exits and Ctrl+Q opens Quit. Close only HO-SHELL-C04 semantic/process assertions; HO-MENUS full parent stays049 and final closure050. Preserve MenuBar key handling before global interrupt/quit, separate paste routing, earlier true-modal capture and retained lifecycle polling. Never suppress global Ctrl+C everywhere to satisfy menu capture.

- **D-001:** Retain main's App/Cx/Ui and runtime ownership rather than resurrecting old Screen or root widget modules. Routing order is too-small, modal, menu, attached/input owner, global chord, focused shell control, page, traversal/Escape; paste has separate source dispatch: true modals capture it, while MenuBar preserves passthrough to the underlying page through shared configured field editing. Tabs and domain work have stable distinct identities.
