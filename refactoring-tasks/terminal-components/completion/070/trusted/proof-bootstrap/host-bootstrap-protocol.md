...
The exact CLI is the host-only six-operation CLI in [`docs/refactoring-plan/proof-contract.md`](../../../../../../docs/refactoring-plan/proof-contract.md). There are no extra qualification-only host operations or permissive flags. Unknown options fail. `--campaign` names a directory containing `campaign.json`. `install --destination` creates `bin/tc-proof-host` with exactly the accepted executable bytes. Later operations run that installed executable.
