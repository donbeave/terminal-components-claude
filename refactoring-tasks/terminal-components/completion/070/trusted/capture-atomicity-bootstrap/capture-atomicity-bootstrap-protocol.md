...
The operator supplies an external protected tui-snap Git object database at an explicit host-local absolute path supplied by the verifier (never a container namespace), containing campaign pin `0a2e490802b7b048cd96349c6af860f8a3a05c3d` (includes merged reviewed head `883d03f19d890bbbf27468798db78b04e85297ac`, tree `dadbaa70facc317cfabb52f0374c1f3cdceb46a1` as ancestor). Build source is extracted from that exact commit, never from mutable working-tree files, a moving branch or a candidate path. The source archive, lock, two fixture sources, compilation argv/results, actual resolved Cargo/Rustc bytes/version/path/toolchain and final executable digests are recorded. The decoder is an added binary only in a fresh private source copy; the pinned lock and all original source bytes remain unchanged. Builds are offline and locked. A missing prerequisite, compiler drift, source mutation, build failure or unsupported Darwin isolation fails preparation; none earns negative-test credit.
