...
schema: task/v5
id: TASK-029
title: "Restore deterministic progress spinner and Meter output"
kind: bugfix
---

# TASK-029 — Restore deterministic progress spinner and Meter output

## Goal

Progress, Spinner and Meter preserve deterministic motion and semantic paint.
...
- `crates/tui/src/components/progress.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/meter.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_029.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** Compare determinate/indeterminate/paused/done/failed progress, spinner phase/gap/inactive and exact oracle clock boundaries. Meter zero/full/no-ratio/series, line/block, readout/on-fill, stale/unknown and thresholds paint exact glyphs/roles at every capability. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Keep shared driver clock; Input::Tick does not advance elapsed time. MeterFillRest Color/ReferenceLift uses accepted ordered Canvas/Surface/Elevated/Field/Popover reference policy only; no generic RGB role inference. Authored tokens and explicit colors win before delayed dimming. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Probe every threshold immediately below/equal/above, fill rounding at tiny width, authored alias collisions, ten-frame ASCII timing and motion wrap. Historical seven-of-sixteen Meter changed digests are not new oracle evidence; preserve source history while comparing independent oracle captures. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed progress-bar and spinner and meter oracle cases and the exact task history clauses
...
```gherkin
Given the live progress-bar and spinner and meter implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane progress-bar and spinner and meter actions and checkpoints
...
- **D-001:** Keep shared driver clock; Input::Tick does not advance elapsed time. MeterFillRest Color/ReferenceLift uses accepted ordered Canvas/Surface/Elevated/Field/Popover reference policy only; no generic RGB role inference. Authored tokens and explicit colors win before delayed dimming.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_029 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
