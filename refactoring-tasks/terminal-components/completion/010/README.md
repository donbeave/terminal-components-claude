...
schema: task/v5
id: TASK-010
title: "Restore runtime focus capture and publication behavior"
kind: bugfix
---

# TASK-010 — Restore runtime focus capture and publication behavior

## Goal

Published focus, capture and typing ownership reproduce oracle event trajectories.
...
- `crates/tui/src/event.rs` and `crates/tui/src/keymap.rs` only for one private effective-chord equivalence shared by matching, all scoped conflict checks and focused-hint deduplication; preserve structural Eq/Hash and case identity. TASK-009 already precedes this event writer and TASK-028 follows this keymap writer.

- `crates/tui/src/runtime.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/runtime/typing.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/focus.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/hit.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/capture.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_010.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** Preserve all twelve pointer_capture_eligibility tests. Successful publication of disabled/absent/decorative/empty/layer-blocked targets cancels held press/capture; aborted publication does not. Test truly overlapping later disabled top over enabled lower on same layer and across layers; neither owner may receive leaked activation. Re-enable never resurrects a gesture; new independent Press works. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Keep one runtime ring/dispatch/presentation authority; input and draw use the same controlled props and allocation. Pending input is delivered only against successfully published geometry. Ui::reference alone suppresses every registration while forcing only exact-target runtime presentation flags. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Probe Press, Release, Click, DoubleClick, DragStart, Drag, DragEnd and Secondary separately, disable mid-drag, release outside, remove actual part, move eligible part, resize and push/pop modal. Preserve hover observation, keyboard hover suppression, boundary wheel, ClickOnly and outside dismissal. Nested focus settling never redelivers physical input; one typing/cursor owner survives route resize. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed focus-hit-capture oracle cases and the exact task history clauses
...
```gherkin
Given the live focus-hit-capture implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane focus-hit-capture actions and checkpoints
...
- **D-001:** Keep one runtime ring/dispatch/presentation authority; input and draw use the same controlled props and allocation. Pending input is delivered only against successfully published geometry. Ui::reference alone suppresses every registration while forcing only exact-target runtime presentation flags.
- **D-002:** Apply ADJ-01 exactly as recorded in architecture-adjudication.md.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_010 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
