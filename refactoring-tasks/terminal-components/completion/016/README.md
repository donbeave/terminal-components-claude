...
schema: task/v5
id: TASK-016
title: "Restore shared field editing and secret lifecycle"
kind: bugfix
---

# TASK-016 — Restore shared field editing and secret lifecycle

## Goal

Fields separate keyboard navigation from completed-click editing and preserve secret lifecycle.
...
- **P-001:** The protected campaign record has accepted TASK-071/072 native qualification products, sealed complete baseline TASK-006 and exact inventory/dispositions TASK-007/008; every listed predecessor is accepted and integrated into the recorded parent. TASK-001 and TASK-070 stay retired fail-closed; former host/runner obligations live in `scripts/campaign-build-proof.sh`, `docs/refactoring-plan/path-contract.md`, `docs/refactoring-plan/proof-contract.md`, and `tools/refactor-proof/{src,runner,bin}`.
...
- `crates/tui/src/components/input.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/textarea.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/field.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/field_control.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/secret.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/validate.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_016.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** Keyboard FocusIn alone remains navigation; completed click enters TextInput/TextArea editing at exact grapheme caret. Press-only then release outside does not edit. Preserve first/repeated click, Enter/typing, horizontal/multiline scroll, selection, commit/cancel/blur/validation and externally controlled value synchronization. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Field supplies chrome only and no second focus stop. Shared TextEditorCore and sealed String/Secret FieldControl own lifecycle; no duplicate app editor. Secret has no Clone/Eq/Serialize, Debug redacts, synthetic mask preserves grapheme geometry and drafts clear on close/cancel/removal. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Exercise disabled versus readonly, masked CJK/emoji/combining hits, invalid TextInput commit ending the edit and recording its validation error without an invented commit/navigation veto, hide/show, type/sensitivity transition and focus loss. Secret sentinel must not occur in logs/copy/frame/error artifacts. Preserve compile-fail containment, safe best-effort zeroization and CP-SCROLL-FADE multiline states. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed field-chrome and text-input and text-area and secret-validation oracle cases and the exact task history clauses
...
```gherkin
Given the live field-chrome and text-input and text-area and secret-validation implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane field-chrome and text-input and text-area and secret-validation actions and checkpoints
...
- **D-001:** Field supplies chrome only and no second focus stop. Shared TextEditorCore and sealed String/Secret FieldControl own lifecycle; no duplicate app editor. Secret has no Clone/Eq/Serialize, Debug redacts, synthetic mask preserves grapheme geometry and drafts clear on close/cancel/removal.
...
- **D-005:** Future `$WORKTREE/tools/refactor-proof/bin/tc-proof` operations are installed from the native proof tree (`scripts/campaign-build-proof.sh`, `tools/refactor-proof/{src,runner,bin}`) and accepted TASK-071 account-tests / TASK-072 architecture qualification receipts. TASK-001 and TASK-070 are retired fail-closed and cannot supply producer receipts. No candidate-local substitute or recursive taskfmt gate is permitted.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_016 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
