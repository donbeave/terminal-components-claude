...
schema: task/v5
id: TASK-028
title: "Restore live status hints and key glyph composition"
kind: bugfix
---

# TASK-028 — Restore live status hints and key glyph composition

## Goal

Status and hints reflect live owner/action metadata with exact truncation.
...
- `trusted/source-witnesses.md` in full; its finite source-state, actual-paint and real-mutant cases are normative R-001/R-002/R-003 proof obligations, not optional examples.
...
- `crates/tui/src/components/status.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/hintbar.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/components/keyhint.rs` solely for the behavior and direct proof stated in this package.
- `crates/tui/src/keymap.rs` solely for shared ActionKey/chord resolution, structural map/table identity, effective descriptors, focused-hint cache and existing typed diagnostics required by A48/A50 and HL-68-HINTS/MAP/PUBLISH.
- `crates/tui/src/ui/mod.rs` solely for shared keymap snapshot/revision, effective descriptor publication and hint-cache invalidation; preserve TASK-073 attribution and timing instrumentation exactly.
- `crates/tui/src/ui/cx.rs` solely for effective owner/action chord access and command identity; retain runtime routing, capture and publication invariants from TASK-010/017.
- `crates/tui/tests/completion_028.rs` solely for the named task-specific regression and mutation witnesses.
...
- **R-001 (MUST):** StatusBar and Segments preserve their distinct source-qualified spacing, width budgets and declaration priority/drop order under D-BRANCH-SOURCE; final-left survivor ellipsis applies only to StatusBar; only keyed hovered label lifts and blank/unkeyed/key input clears hover. HintBar action/chord priority and overflow marker; KeyHint modifier fragments, remaps and multi-key clipping match handled actions. All source-qualified historical clauses assigned to this task and listed in trusted/obligations.md are binding with their recorded accepted/superseded/deferred dispositions. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-002 (MUST):** Retain one StatusBar for old segments/statusbar, pure draw-time caller projection with fixed buffers and no mutable mirror cache. Binding metadata and structural revisions drive actual routing and hints, warm caches allocate zero. Preserve the architectural and CP-COMMON clauses in trusted/obligations.md, using the same production owner for pixels and interaction. Apply every corresponding row in `trusted/source-obligations.tsv` according to its exact source-qualified disposition and assigned acceptance/check mapping; accepted clauses require implementation or preservation proof, and rejected/superseded/deferred alternatives require their stated non-goal or replacement proof.
- **R-003 (MUST):** Probe active/inactive/modal/edit owners, remap/remove/hidden/latent/duplicate chord, long mandatory label at tiny width, offsets and live inactive state update before pure repaint. Compare glyph/foreground/background/modifiers, not just distinct hashes; no hint points to unhandled action. Execute every required historical/oracle test identity under the immutable stage map; compatible and previously closed cases stay green, all future-owned failures remain recorded and no case is silently omitted.
...
```gherkin
Given the sealed statusbar-segments and hintbar and keyhint oracle cases and the exact task history clauses
...
```gherkin
Given the live statusbar-segments and hintbar and keyhint implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane statusbar-segments and hintbar and keyhint actions and checkpoints
...
- **D-BINDING-OWNER:** TASK-028 is the single production owner of the shared binding/cache boundary needed by TASK-030; no component-local resolver or revision mirror is permitted. TASK-073 precedes this task and owns observation/timing infrastructure. TASK-010 precedes through TASK-027; TASK-017 is an explicit prerequisite so boundary probes observe the accepted layer policy. TASK-030 consumes this accepted boundary and tests help/Wizard integration without changing it. Preserve explicit application routing policy and source-qualified oracle exceptions; fixing metadata must not invent newly handled product chords.

- **D-001:** Retain one StatusBar for old segments/statusbar, pure draw-time caller projection with fixed buffers and no mutable mirror cache. Binding metadata and structural revisions drive actual routing and hints, warm caches allocate zero.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_028 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
