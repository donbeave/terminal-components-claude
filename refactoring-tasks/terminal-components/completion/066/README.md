...
schema: task/v5
id: TASK-066
title: "Close exact test preservation and render-target relocation"
kind: refactor
---

# TASK-066 — Close exact test preservation and render-target relocation

## Goal

Consolidate the expired render/render_components split into one test target while preserving every exact source-qualified test, profile, scenario key and assertion through an approved relocation map; close the complete required execution inventory.
...
- **P-001:** The host has accepted and integrated every hard dependency: `terminal-components/completion/007`, `terminal-components/completion/039`, `terminal-components/completion/050`, `terminal-components/completion/057`, `terminal-components/completion/064`, `terminal-components/completion/065`. Their tested source identities must be ancestors of the actual integration parent. TASK-065 precedes this task because both write xtask/src/main.rs; canonical dependency enforcement serializes the shared owner.
- **P-002:** The accepted TASK-071/TASK-072 native qualification products, sealed complete oracle bundle, required-test inventory and dispositions are installed read-only. TASK-001 and TASK-070 stay retired fail-closed; former host/runner obligations live in `scripts/campaign-build-proof.sh`, `docs/refactoring-plan/path-contract.md`, `docs/refactoring-plan/proof-contract.md`, and `tools/refactor-proof/{src,runner,bin}`. The verifier has recorded this package and generated `$RUN_DIR/context-index.json` and `$RUN_DIR/contexts/CHK-NNN.json` (immutable per-check contexts) from those receipts.
...
- **R-001 (MUST):** Consolidate the expired render/render_components split into one test target while preserving every exact source-qualified test, profile, scenario key and assertion through an approved relocation map; close the complete required execution inventory. Every numbered closure clause in `trusted/obligations.md` and every mapped exact historical clause in `trusted/source-obligations.tsv` must have its designated passing independent proof. Preserve complete direct and PTY oracle equality for all completed product scenarios.
- **R-002 (MUST):** Preserve fixed original archives and accepted oracle replacements separately. Moving an assertion never changes its meaning. Preserve primary-toolchain compile-fail assertions and the existing MSRV profile assignment; test counts and cargo listings are not execution proof.
...
- **D-001:** This task is the explicit reviewed relocation producer, not general permission to rewrite tests. The host validates old and new assertion/source identities independently before accepting its relocation receipt. Historical archive bytes and expected oracle evidence remain protected.
...
- [ ] **2** Complete the bounded closure.
    - [ ] **2.1** Deliver every exact closure clause and its independent proof. (`R-001`, `AC-001`, `CHK-005`)
