...
schema: task/v5
id: TASK-013
title: "Restore logical grapheme display copy and editor grammar"
kind: bugfix
---

# TASK-013 — Restore logical grapheme display copy and editor grammar

## Goal

One logical grapheme and editor grammar governs display, movement and source-preserving copy.
...
- `crates/tui/src/ui/paint.rs` solely for the bounded shared implementation owner in this task's fixed contract.
- `crates/tui/src/ui/mod.rs` solely for one private per-FrameState reusable text-cluster scratch field and its safe scoped ownership; no public Runtime API, global scratch or published frame/state payload.

- `crates/tui/src/text` solely for the behavior and direct proof stated in this package.
- `crates/tui/tests/completion_013.rs` solely for the named task-specific regression and mutation witnesses.
...
```gherkin
Given the sealed text-core oracle cases and the exact task history clauses
...
```gherkin
Given the live text-core implementation and independent ownership witnesses
...
```gherkin
Given the fixed direct-lane text-core actions and checkpoints
...
- **D-001:** Preserve borrowed TextEditorCore/TextBuffer/CellWidth ownership and allocation-free clipping; no per-widget alternative Unicode algorithm or preformatted owned span copies.
...
- [ ] **2** Deliver the owned component outcome.
    - [ ] **2.1** Reproduce the explicit oracle boundaries and add completion_013 regression witnesses. (`R-001`, `AC-001`, `CHK-004`)
