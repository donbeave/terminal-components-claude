...
impl PendingEdits {
    ...
    pub fn delete_row(&mut self, row: usize) -> bool {
        ...
        if self.deleted.get(row).copied().unwrap_or(false) {
            return false;
        }
        if let Some(deleted) = self.deleted.get_mut(row) {
            *deleted = true;
        }
        true
    }
...
impl ResultGrid {
    ...
    /// Commit one typed cell edit and retain an undo snapshot.
    ///
    /// # Errors
    ///
    /// Returns a field error when the text cannot be parsed for the column's
    /// declared type.
    pub fn commit_cell(&mut self, row: usize, col: usize, text: &str) -> Result<(), FieldError> {
    ...
    fn reorder_display(&mut self, order: &[usize]) {
        let width = self.types.len();
        if width == 0 {
            return;
        }
        let rows = self.display.len().checked_div(width).unwrap_or(0);
        let mut inverse = vec![0usize; rows];
        for (new, &old) in order.iter().enumerate() {
            if old < rows
                && let Some(slot) = inverse.get_mut(old)
            {
                *slot = new;
            }
        }
        for index in 0..rows {
            while inverse.get(index).copied().unwrap_or(index) != index {
                let destination = inverse.get(index).copied().unwrap_or(index);
                if destination >= rows {
                    break;
                }
                for column in 0..width {
                    self.display.swap(
                        index.saturating_mul(width).saturating_add(column),
                        destination.saturating_mul(width).saturating_add(column),
                    );
                }
                inverse.swap(index, destination);
            }
        }
    }
