...
pub fn form_fields() -> [FieldSpec<'static>; 15] {
    ...
    [
        ...
        FieldSpec::new(HOST, "Host", FieldKind::Text(TextInput::new(HOST).placeholder("localhost"))).span(FieldSpan::Half).group(BASIC),
        FieldSpec::new(PORT, "Port", FieldKind::Text(TextInput::new(PORT).validate(&valid_port))).span(FieldSpan::Half).group(BASIC),
        FieldSpec::new(DATABASE, "Database", FieldKind::Text(TextInput::new(DATABASE))).group(BASIC),
        FieldSpec::new(USER, "Username", FieldKind::Text(TextInput::new(USER))).group(BASIC),
        FieldSpec::new(PASSWORD, "Password", FieldKind::Text(TextInput::new(PASSWORD).secret(SecretPolicy::default()))).help("Never written to connections.json").group(BASIC),
        FieldSpec::new(ASK_PASSWORD, "", FieldKind::Check(Checkbox::new(ASK_PASSWORD, "Prompt for password on connect"))).plain(true).group(BASIC),
        FieldSpec::new(ENVIRONMENT, "Environment", FieldKind::Radio(RadioGroup::new(ENVIRONMENT))).group(BASIC),
        FieldSpec::new(GROUP, "Group", FieldKind::Select(Select::new(GROUP))).group(BASIC),
        FieldSpec::new(SAFE_MODE, "Safe Mode", FieldKind::Radio(RadioGroup::new(SAFE_MODE))).group(BASIC),
        FieldSpec::new(SSL, "", FieldKind::Toggle(Toggle::new(SSL, "Use SSL / TLS"))).plain(true).group(ADVANCED),
        FieldSpec::new(SSH, "", FieldKind::Toggle(Toggle::new(SSH, "SSH tunnel"))).plain(true).group(ADVANCED),
        FieldSpec::new(SSH_HOST, "SSH host", FieldKind::Text(TextInput::new(SSH_HOST).placeholder("bastion.example.com"))).group(ADVANCED),
        FieldSpec::new(STARTUP, "Startup commands", FieldKind::Area(TextArea::new(STARTUP, 3))).help("Run after every connect, one per line").group(ADVANCED),
    ]
}
...
pub fn form_actions() -> [Action<'static>; 4] {
    [
        ...
        Action::new(ActionKey::SAVE, "Save").chord(Chord::with(tui_next::KeyCode::Char('s'), tui_next::KeyModifiers::CONTROL)),
...
impl ConnectionDraft {
    ...
    pub fn from_connection(connection: &Connection) -> Self {
        Self {
            ...
            engine: match connection.engine { Engine::Postgres => 0, Engine::MySql => 1, Engine::Sqlite => 2 },
            ...
            ask_password: false,
            environment: match connection.environment { Environment::Local => 0, Environment::Development => 1, Environment::Staging => 2, Environment::Production => 3 },
            group: GROUPS.iter().position(|g| g.eq_ignore_ascii_case(&connection.group)).unwrap_or(0),
            safe_mode: SafeMode::ALL.iter().position(|mode| *mode == connection.safe_mode).unwrap_or(0),
            ssl: connection.ssl,
    ...
    pub fn validate_all(&self) -> Result<(), (Id, tui_next::FieldError)> {
        ...
        if self.name.trim().is_empty() {
            return Err((NAME, tui_next::FieldError::coded("Name is required", "required")));
        }
        if self.engine == 0 && self.database.trim().is_empty() {
            return Err((DATABASE, tui_next::FieldError::coded("Database is required", "required")));
        }
    ...
    pub fn to_connection(&self, base: Option<&Connection>) -> Option<Connection> {
        let port = if self.port.is_empty() { default_port(self.engine).parse().ok()? } else { self.port.parse().ok()? };
        let engine = match self.engine { 0 => Engine::Postgres, 1 => Engine::MySql, 2 => Engine::Sqlite, _ => return None };
        let environment = match self.environment { 0 => Environment::Local, 1 => Environment::Development, 2 => Environment::Staging, 3 => Environment::Production, _ => return None };
        Some(Connection {
            name: self.name.clone(), engine, host: self.host.clone(), port,
            database: self.database.clone(), user: self.user.clone(), environment,
            safe_mode: *SafeMode::ALL.get(self.safe_mode)?, ssl: self.ssl,
            ssh: self.ssh.then(|| self.ssh_host.clone()),
            group: GROUPS.get(self.group).copied().or_else(|| GROUPS.first().copied()).unwrap_or("Personal").to_owned(),
    ...
    pub fn password_status(&self) -> &'static str { if self.has_password() { "saved" } else { "not set" } }

    fn options(id: Id) -> &'static [&'static str] {
        match id { field::ENGINE => ENGINES, field::ENVIRONMENT => ENVIRONMENTS, field::GROUP => GROUPS, field::SAFE_MODE => SAFE_MODES, _ => &[] }
    }
}
...
impl FormData for ConnectionDraft {
    fn value(&self, id: Id) -> FieldRef<'_> {
        use field::*;
        match id {
            NAME => FieldRef::Text(&self.name), ENGINE => FieldRef::Choice(self.engine), HOST => FieldRef::Text(&self.host), PORT => FieldRef::Text(&self.port), DATABASE => FieldRef::Text(&self.database), USER => FieldRef::Text(&self.user), PASSWORD => FieldRef::Secret(&self.password), ASK_PASSWORD => FieldRef::Flag(self.ask_password), ENVIRONMENT => FieldRef::Choice(self.environment), GROUP => FieldRef::Choice(self.group), SAFE_MODE => FieldRef::Choice(self.safe_mode), SSL => FieldRef::Flag(self.ssl), SSH => FieldRef::Flag(self.ssh), SSH_HOST => FieldRef::Text(&self.ssh_host), STARTUP => FieldRef::Text(&self.startup), _ => FieldRef::Text(""),
        }
    }
    ...
    fn value_mut(&mut self, id: Id) -> FieldMut<'_> {
        use field::*;
        match id {
            NAME => FieldMut::Text(&mut self.name), ENGINE => FieldMut::Choice(&mut self.engine), HOST => FieldMut::Text(&mut self.host), PORT => FieldMut::Text(&mut self.port), DATABASE => FieldMut::Text(&mut self.database), USER => FieldMut::Text(&mut self.user), PASSWORD => FieldMut::Secret(&mut self.password), ASK_PASSWORD => FieldMut::Flag(&mut self.ask_password), ENVIRONMENT => FieldMut::Choice(&mut self.environment), GROUP => FieldMut::Choice(&mut self.group), SAFE_MODE => FieldMut::Choice(&mut self.safe_mode), SSL => FieldMut::Flag(&mut self.ssl), SSH => FieldMut::Flag(&mut self.ssh), SSH_HOST => FieldMut::Text(&mut self.ssh_host), STARTUP => FieldMut::Text(&mut self.startup), _ => FieldMut::ReadOnly,
        }
    }
    ...
    fn visible(&self, id: Id) -> bool { id != field::SSH_HOST || self.ssh }
    fn disabled(&self, id: Id) -> bool { id == field::PASSWORD && self.ask_password }
    fn validate(&self, id: Id, value: FieldRef<'_>) -> Result<(), tui_next::FieldError> {
        if id == field::PORT && let FieldRef::Text(text) = value { valid_port(text) } else { Ok(()) }
    }
    fn validate_all(&self) -> Result<(), (Id, tui_next::FieldError)> { Self::validate_all(self) }
}
...
impl ConnectionsScreen {
    ...
    pub fn new(connections: Vec<Connection>) -> Self { Self { connections, selected: 0, filter: String::new(), error: None } }
    /// Build the demo list.
    pub fn default_list() -> Self { Self::new(db::connections()) }
    /// Return rows visible under the current filter.
    pub fn visible(&self) -> Vec<(usize, &Connection)> {
        let q = self.filter.to_ascii_lowercase();
        self.connections.iter().enumerate().filter(|(_, c)| q.is_empty() || c.name.to_ascii_lowercase().contains(&q)).collect()
    }
    ...
    pub fn connect_selected(&mut self) -> Option<Connection> {
        ...
        match connection.outcome {
            ConnectOutcome::Ok => { self.error = None; Some(connection) }
            ConnectOutcome::AuthFailed => { self.error = Some("Authentication failed; press r to retry".to_owned()); None }
            ConnectOutcome::Unreachable => { self.error = Some("Connection unreachable; press r to retry".to_owned()); None }
        }
    }
    /// Retry the selected row.
    pub fn retry(&mut self) -> Option<Connection> { self.connect_selected() }
}
