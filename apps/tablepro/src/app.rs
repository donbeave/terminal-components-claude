...
impl TableProApp {
    ...
    fn tab_strip<'a>() -> Tabs<
        'a,
        String,
        impl Fn(&String) -> tui_next::ItemKey,
        impl Fn(&String, &mut tui_next::RowUi<'_>),
    > {
        Tabs::<String>::new(TAB_STRIP)
        ...
        ) && let Some(command) = cx.command()
        {
            match command {
                ...
                c if c == SAFE_MODE => {
                    ...
                    self.set_safe_mode(
                        modes
                            .get(current.saturating_add(1) % modes.len())
                            .copied()
