...
impl QueryTab {
    ...
    /// # Errors
    ///
    /// Returns a parse, statement-kind, or execution error.
    ...
    /// # Errors
    ///
    /// Returns a parse, statement-kind, or planning error.
...
impl Tab {
    /// Borrow the stable label without allocating a new tab title.
    pub fn label_ref(&self) -> &str {
        match self {
            Self::Table(tab) => tab.table.name.as_str(),
            Self::Query(tab) => tab.name.as_str(),
