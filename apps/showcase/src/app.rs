    ...
    fn update(&mut self, cx: &mut Cx<'_>) -> Response<()> {
        ...
        match command {
            ...
            Some(HELP_COMMAND) if !cx.is_open(HELP) => {
                // Keep the page registry alive below this diagnostic modal,
                // matching the legacy benchmark's shadowed-page workload.
                cx.open_layer(HELP, Self::help_dialog().layer(cx).inert_below(false));
            }
            _ => {}
        }
