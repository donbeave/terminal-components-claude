...
use tui_next::{Button, Cx, Dialog, DialogAction, DialogState, Id, ItemKey, List, ListAction, ListState, Rect, Response, RowUi, Tabs, TabsAction, TabsState, Ui, Variant, id, layout};
...
struct Member { id: u8, name: &'static str, email: &'static str }
...
fn member_row(member: &Member, row: &mut RowUi<'_>) { row.label(member.name); row.meta(member.email); }
fn member_list() -> List<'static, Member, impl Fn(&Member) -> ItemKey, impl Fn(&Member, &mut RowUi<'_>)> { List::new(MEMBERS).key(member_key).row(member_row) }

fn remove_dialog() -> Dialog<'static> { Dialog::destructive(REMOVE_DIALOG, "Remove member?", "This member will lose access to the workspace.") }
...
impl SettingsPage {
    pub(crate) fn new() -> Self {
        Self { tabs: TabsState::default(), members: INITIAL_MEMBERS.to_vec(), member_state: ListState::default(), remove_state: DialogState::default(), remove_open: false, selected: 0, message: "workspace settings" }
    }

    fn selected_member(&self) -> Option<&Member> { self.members.get(self.selected) }
}

impl Default for SettingsPage { fn default() -> Self { Self::new() } }
...
impl Page for SettingsPage {
    ...
    fn update(&mut self, cx: &mut Cx<'_>) -> Response<()> {
        ...
        if let Some(TabsAction::Activated(key)) = tabs.action_ref() {
            self.message = match key { ItemKey::Index(0) => "general settings", ItemKey::Index(1) => "members", ItemKey::Index(2) => "security", _ => "tab changed" };
        }
        result |= tabs.erase();
        ...
        if let Some(ListAction::Chose(key) | ListAction::Activated(key)) = list.action_ref()
            && let Some(index) = self.members.iter().position(|member| member_key(member) == *key)
        {
            self.selected = index;
            self.message = "member selected";
        }
        result |= list.erase();
        let invite = Button::new(INVITE, "Invite member").variant(Variant::SECONDARY).update(cx);
        if invite.activated() { self.message = "invite flow ready"; }
        result |= invite.erase();
        let remove = Button::new(REMOVE, "Remove member").variant(Variant::DANGER).disabled(self.members.is_empty()).update(cx);
    ...
    fn draw(&self, ui: &mut Ui<'_>, area: Rect) {
        frame(ui, area, self.title(), "tabs · keyed members · destructive flow", |ui, body| {
            ...
            Button::new(INVITE, "Invite member").variant(Variant::SECONDARY).draw(ui, action_rows.first().copied().unwrap_or(actions));
            Button::new(REMOVE, "Remove member").variant(Variant::DANGER).disabled(self.members.is_empty()).draw(ui, action_rows.get(1).copied().unwrap_or(actions));
            let count = format!("{} members · {}", self.members.len(), self.message);
            let _ = ui.paint_str(action_rows.get(2).copied().unwrap_or(actions), &count, ui.surface_style());
            lines(ui, Rect { y: actions.bottom().saturating_add(1), height: 1, ..body }, &["Member removal is confirmed in a trapped modal and updates the domain list."]);
        });
        ui.layer(REMOVE_DIALOG, |ui, layer| { remove_dialog().draw(ui, layer, &self.remove_state, |ui, body| { let _ = ui.paint_str(body, "Enter confirms · Esc keeps the member", ui.surface_style()); }); });
    }
}
