...
impl Route {
    const fn tick_ms(self, animating: bool) -> u64 {
        match self {
...
impl App {
    /// Pure fixture entry point at frame zero using the Junie theme.
    pub fn for_scenario(scenario: Scenario, motion: Motion) -> Self {
        Self::for_scenario_with_theme(scenario, motion, 0, Theme::junie())
    }

    /// Pure fixture entry point at a pinned frame using the Junie theme.
    pub fn for_scenario_at(scenario: Scenario, motion: Motion, frame: u64) -> Self {
        Self::for_scenario_with_theme(scenario, motion, frame, Theme::junie())
    }

    /// Pure fixture entry point with an explicit app theme.
    pub fn for_scenario_with_theme(
        scenario: Scenario,
        motion: Motion,
        frame: u64,
        theme: Theme,
    ...
    /// Deterministic cadence used by the current route.
    pub const fn route_tick_ms(&self) -> u64 {
        self.route.tick_ms(false)
    }

    /// Current pinned fixture frame.
    pub fn frame(&self) -> u64 {
        match self.route {
            Route::Intro => self.intro.as_ref().map_or(self.fixture_frame, |s| s.tick),
            Route::Outro => self.outro.as_ref().map_or(self.fixture_frame, |s| s.tick),
            Route::Handoff => self.handoff.unwrap_or(self.fixture_frame),
            Route::Cockpit => self
                .screens
                .cockpit
                .as_ref()
                .map_or(self.fixture_frame, |s| s.run.tick),
            _ => self.fixture_frame,
        }
    }

    /// Selected role label used by the default launch fixture.
    pub const fn selected_role(&self) -> &'static str {
        "chainargos/the-architect"
    }

    /// Active launch run, when the launch cockpit exists.
    pub fn launch(&self) -> Option<&crate::sim::launch::LaunchRun> {
        self.screens.cockpit.as_ref().map(|screen| &screen.run)
    }
    ...
    fn draw_frame(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut RenderCtx, route: Route) {
        ...
        if let Some(s) = self.screens.get_mut(route) {
            s.render(body, buf, ctx, &mut self.world);
        }
